...
fn create_extended_test_configs(temp_dir: &TempDir) -> (WorkContinuityConfig, PersistenceConfig, CoordinationConfig) {
...
async fn test_autonomous_workflow_persistence_across_restarts() -> Result<()> {
    ...
    let (continuity_config, persistence_config, coordination_config) = create_extended_test_configs(&temp_dir);
    ...
    let (saved_checkpoint_id, test_issue) = {
        ...
        // Save state
        let checkpoint_id = persistence_manager.save_state(&persistent_state, CheckpointReason::UserRequested).await?;
        
        println!("Saved autonomous workflow state with checkpoint ID: {}", checkpoint_id);
    ...
    // Phase 2: Simulate restart - load state and verify recovery
    {
        ...
        match loaded_state.current_state.unwrap() {
            AutonomousWorkflowState::UnderReview { issue, pr, .. } => {
                ...
                println!("Successfully recovered UnderReview state for issue #{}", issue.number);
            }
            other => panic!("Unexpected recovered state: {:?}", other),
        }

        // Verify state history was preserved
        assert!(loaded_state.state_history.len() >= 5, 
               "State history not preserved: {} entries", loaded_state.state_history.len());
        
        // Verify checkpoint metadata
        assert_eq!(loaded_state.checkpoint_metadata.checkpoint_id, "restart-test-checkpoint");
...
async fn test_autonomous_coordinator_with_work_continuity() -> Result<()> {
    ...
    // Phase 2: Simulate process restart and recovery
    {
        ...
        println!("  Pending operations: {}", continuity_status.pending_operations);
    }
...
async fn test_checkpoint_recovery_scenarios() -> Result<()> {
    ...
    // Test Scenario 1: Multiple checkpoints with different states
    {
        ...
        for (state_name, workflow_state) in states_to_test {
            ...
            let checkpoint_id = persistence_manager.save_state(&persistent_state, CheckpointReason::PeriodicSave).await?;
        ...
        for (state_name, checkpoint_id, test_agent_id) in checkpoint_ids {
            ...
            assert!(loaded_state.is_some(), "Failed to load {} state", state_name);
            
            let loaded_state = loaded_state.unwrap();
            assert!(loaded_state.current_state.is_some(), "{} state not preserved", state_name);
            
            println!("Successfully recovered {} state for agent {}", state_name, test_agent_id);
        }
    }
...
async fn test_checkpoint_performance() -> Result<()> {
    ...
    println!("Testing checkpoint/recovery performance over {} iterations", test_iterations);
    ...
    for i in 0..test_iterations {
        ...
        let complex_state = PersistentWorkflowState {
            ...
            max_work_hours: 8,
            state_history: (0..i).map(|j| {
                my_little_soda::autonomous::StateTransitionRecord {
        ...
        let _persist_checkpoint_id = persistence_manager.save_state(&complex_state, CheckpointReason::PeriodicSave).await?;
        ...
        if i % 3 == 0 {
            println!("Iteration {}: Checkpoint {:?}, Recovery {:?}, Action: {:?}", 
                    i, checkpoint_duration, recovery_duration, resume_action.is_some());
        }
    ...
    let avg_checkpoint_time = checkpoint_times.iter().sum::<Duration>() / checkpoint_times.len() as u32;
    ...
    assert!(avg_checkpoint_time < Duration::from_millis(500), 
           "Average checkpoint time too slow: {:?}", avg_checkpoint_time);
    assert!(max_checkpoint_time < Duration::from_secs(2), 
           "Max checkpoint time too slow: {:?}", max_checkpoint_time);
    assert!(avg_recovery_time < Duration::from_millis(100), 
           "Average recovery time too slow: {:?}", avg_recovery_time);
    assert!(max_recovery_time < Duration::from_millis(500), 
           "Max recovery time too slow: {:?}", max_recovery_time);
...
async fn test_workflow_state_continuity() -> Result<()> {
    ...
    let test_states = vec![
        ("assigned", Issue { number: 301, title: "Assigned continuity test".to_string(), body: "".to_string(), labels: vec![], priority: Priority::Low, estimated_hours: Some(1) }),
        ("in-progress", Issue { number: 302, title: "In-progress continuity test".to_string(), body: "".to_string(), labels: vec![], priority: Priority::Medium, estimated_hours: Some(2) }),
        ("review", Issue { number: 303, title: "Review continuity test".to_string(), body: "".to_string(), labels: vec![], priority: Priority::High, estimated_hours: Some(3) }),
    ];
    ...
    for (state_name, test_issue) in test_states {
        ...
        // Verify recovery
        let recovered_state = persistence_manager.load_state(&format!("{}-{}", agent_id, state_name)).await?;
        assert!(recovered_state.is_some(), "Failed to recover {} state", state_name);
        
        let recovered_state = recovered_state.unwrap();
        assert!(recovered_state.current_state.is_some(), "{} workflow state not preserved", state_name);
        ...
        match recovered_state.current_state.unwrap() {
            ...
            AutonomousWorkflowState::InProgress { issue, progress, .. } if state_name == "in-progress" => {
                assert_eq!(issue.number, test_issue.number);
