...
use fixtures::test_metrics_collector::{
    BatchMetricsCollector, SimpleTestTimer, TestMetricsCollector,
};
...
async fn test_comprehensive_test_result_reporting_system() -> Result<()> {
    ...
    let failure_names: Vec<String> = summary
        .failed_test_details
        .iter()
...
async fn test_real_test_execution_with_reporting() -> Result<()> {
    ...
    println!(
        "   Generated {} artifacts for real test execution",
        artifacts.len()
    );
    for artifact in &artifacts {
        assert!(std::path::Path::new(artifact).exists());
...
async fn test_test_summary_dashboard_generation() -> Result<()> {
    ...
    println!(
        "     Performance Score: {}",
        calculate_performance_score(&summary)
    );
...
async fn run_failing_test_with_metrics(test_name: &str, category: &str) -> Result<TestResult> {
    ...
    let error_msg = format!(
        "Simulated failure in {}: validation check failed",
        test_name
    );
...
fn calculate_performance_score(
    summary: &fixtures::test_result_reporting::TestSummaryReport,
) -> String {
    let avg_time_ms = summary
