...
async fn test_git_operation_failure_recovery() {
    ...
    assert!(matches!(push_strategy, RecoveryStrategy::RetryWithBackoff { .. }));
    ...
    assert!(matches!(pull_strategy, RecoveryStrategy::RetryWithBackoff { .. }));
...
async fn test_build_failure_recovery() {
    ...
    assert!(matches!(compile_strategy, RecoveryStrategy::AutomatedFix { .. }));
...
async fn test_test_failure_recovery() {
    ...
    let unit_test_error = ErrorType::TestFailure {
        ...
        failed_tests: vec!["test_authentication".to_string(), "test_validation".to_string()],
    };
    
    let unit_strategy = error_recovery.determine_recovery_strategy(&unit_test_error);
    assert!(matches!(unit_strategy, RecoveryStrategy::AutomatedFix { .. }));
    ...
    assert!(matches!(integration_strategy, 
        RecoveryStrategy::AutomatedFix { .. } |
        RecoveryStrategy::Escalate { .. }
    ));
...
async fn test_merge_conflict_recovery() {
    ...
    assert!(matches!(simple_strategy, RecoveryStrategy::AutomatedFix { 
    ...
    assert!(matches!(complex_strategy, 
        RecoveryStrategy::AutomatedFix { confidence: ConfidenceLevel::Low, .. } |
        RecoveryStrategy::Escalate { .. }
    ));
    
    // Test binary file merge conflict recovery
    let binary_conflict = ErrorType::MergeConflict {
        files: vec!["assets/image.png".to_string(), "docs/diagram.pdf".to_string()],
        conflict_count: 2,
    };
    ...
    assert!(matches!(binary_strategy, 
        RecoveryStrategy::Escalate { .. } |
        RecoveryStrategy::AutomatedFix { confidence: ConfidenceLevel::Low, .. }
    ));
}
...
async fn test_dependency_failure_recovery() {
    ...
    assert!(matches!(missing_strategy, RecoveryStrategy::AutomatedFix { .. }));
    ...
    assert!(matches!(version_strategy, 
        RecoveryStrategy::AutomatedFix { .. } |
        RecoveryStrategy::Escalate { .. }
    ));
    ...
    assert!(matches!(security_strategy, RecoveryStrategy::AutomatedFix { .. }));
}
...
async fn test_recovery_execution_scenarios() {
    ...
    let automated_result = error_recovery.execute_recovery_strategy(
        automated_error.clone(),
        automated_strategy,
        &test_state,
    ).await;
    
    assert!(automated_result.is_ok(), "Automated recovery failed: {:?}", automated_result);
    ...
    let retry_result = error_recovery.execute_recovery_strategy(
        network_error,
        retry_strategy,
        &test_state,
    ).await;
...
async fn test_escalation_scenarios() {
    ...
    assert!(matches!(security_strategy, 
        RecoveryStrategy::AutomatedFix { .. } |
        RecoveryStrategy::Escalate { .. }
    ));
...
async fn test_workflow_error_recovery_integration() {
    ...
    let blocker_result = workflow.handle_event(AutonomousEvent::EncounterBlocker {
        blocker: recoverable_blocker.clone(),
    }).await;
    
    assert!(blocker_result.is_ok(), "Blocker encounter failed: {:?}", blocker_result);
    assert!(matches!(workflow.current_state(), Some(AutonomousWorkflowState::Blocked { .. })));
    
    // Test recovery from blocker
    let resolve_result = workflow.handle_event(AutonomousEvent::ResolveBlocker).await;
    assert!(resolve_result.is_ok(), "Blocker resolution failed: {:?}", resolve_result);
    assert!(matches!(workflow.current_state(), Some(AutonomousWorkflowState::InProgress { .. })));
    ...
    let abandon_result = workflow.handle_event(AutonomousEvent::ForceAbandon {
    ...
    assert!(matches!(workflow.current_state(), Some(AutonomousWorkflowState::Abandoned { .. })));
}
...
async fn test_ci_cd_failure_recovery() {
    ...
    workflow.handle_event(AutonomousEvent::StartWork).await.unwrap();
    workflow.handle_event(AutonomousEvent::CompleteWork).await.unwrap();
    ...
    workflow.handle_event(AutonomousEvent::SubmitForReview { pr }).await.unwrap();
    workflow.handle_event(AutonomousEvent::ApprovalReceived).await.unwrap();
    
    // Test recoverable CI failure
    let recoverable_ci_failure = vec![
        CIFailure {
    ...
    let ci_failure_result = workflow.handle_event(AutonomousEvent::CIFailureDetected {
        failures: recoverable_ci_failure,
    }).await;
    
    assert!(ci_failure_result.is_ok(), "CI failure handling failed: {:?}", ci_failure_result);
    assert!(matches!(workflow.current_state(), Some(AutonomousWorkflowState::CIFailure { .. })));
    ...
    assert!(matches!(workflow.current_state(), Some(AutonomousWorkflowState::Approved { .. })));
    
    // Test unrecoverable CI failure
    let unrecoverable_ci_failure = vec![
        CIFailure {
    ...
    workflow.handle_event(AutonomousEvent::CIFailureDetected {
...
async fn test_recovery_tracking_and_reporting() {
    ...
    let test_state = AutonomousWorkflowState::Blocked {
        ...
        blocker: BlockerType::BuildFailure { error: "test error".to_string() },
    };
    ...
    for (error, strategy) in errors_and_strategies {
        let result = error_recovery.execute_recovery_strategy(
            error,
            strategy,
            &test_state,
        ).await;
        ...
        match result {
            Ok(attempt) => {
                ...
                println!("Recovery attempt {} successful: {}", total_attempts, attempt.success);
            }
            Err(e) => {
    ...
    println!("  Successful attempts: {}", final_report.successful_attempts);
    println!("  Failed attempts: {}", final_report.failed_attempts);
    ...
    assert_eq!(final_report.total_attempts, 
               final_report.successful_attempts + final_report.failed_attempts);
}
...
async fn test_recovery_timeout_handling() {
    ...
    let error_recovery = AutonomousErrorRecovery::new(github_client, base_recovery)
        .with_timeout(1); // 1 minute timeout for testing
    ...
    let timeout_result = error_recovery.execute_recovery_strategy(
        timeout_error,
        timeout_strategy,
        &test_state,
    ).await;
