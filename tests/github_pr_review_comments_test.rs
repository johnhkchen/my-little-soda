...
async fn test_comment_handler_creation() {
    ...
    let comment_handler =
        CommentHandler::new(octocrab, "test-owner".to_string(), "test-repo".to_string());

    // Test that the handler was created successfully
    // This is a basic compilation and structure test
    assert_eq!(
        std::mem::size_of_val(&comment_handler),
        std::mem::size_of::<CommentHandler>()
    );
}
    ...
    fn _assert_methods_exist() {
        ...
        where
            F: Fn(
                ...
                dyn Future<
                        Output = Result<
                            octocrab::models::pulls::Comment,
                            my_little_soda::github::errors::GitHubError,
                        >,
                    > + Send
                    + Unpin,
            >,
        ...
        where
            F: Fn(
                ...
                dyn Future<
                        Output = Result<
                            Vec<octocrab::models::pulls::Comment>,
                            my_little_soda::github::errors::GitHubError,
                        >,
                    > + Send
                    + Unpin,
        ...
        where
            F: Fn(
                ...
                dyn Future<Output = Result<(), my_little_soda::github::errors::GitHubError>>
                    + Send
                    + Unpin,
            >,
        {
