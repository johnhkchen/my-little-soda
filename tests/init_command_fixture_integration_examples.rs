...
async fn example_init_on_empty_repository() {
    ...
    assert!(
        ...
        "Empty repository should have README.md"
    );
    assert!(
        !env.has_file("my-little-soda.toml"),
        "Should not have clambake config initially"
    );
    assert!(
        !env.has_file(".my-little-soda"),
        "Should not have .my-little-soda directory initially"
    );
...
async fn example_batch_testing_dry_run() {
    ...
    let results = InitCommandBatchTester::test_scenario_across_fixtures(scenario)
        .await
    ...
    println!(
        "✅ Example 3 passed: Batch testing with {} fixtures",
        results.results.len()
    );
}
...
async fn example_multi_agent_configuration() {
    ...
    for agent_count in &agent_counts {
        ...
        assert!(
            result.success,
            "Init should succeed with {} agents",
            agent_count
        );
...
async fn example_post_init_state_validation() {
    ...
    // Verify initial state
    assert!(
        env.has_file("Cargo.toml"),
        "Should have existing Cargo.toml"
    );
    ...
    let command_result = env
        .run_and_validate_init(1, false, false)
        .await
        .expect("Failed to run init command");

    // Validate post-init state
    let post_init_validation = env
        .validate_post_init_state(false)
        .expect("Failed to validate post-init state");

    assertions::assert_result_matches_expectation(
        &command_result,
        "repository_with_existing_files",
    );
    assertions::assert_post_init_validation_passes(
    ...
    assert!(
        env.has_file("Cargo.toml"),
        "Existing Cargo.toml should be preserved"
    );
    assert!(
        env.has_file("src/main.rs"),
        "Existing main.rs should be preserved"
    );
    ...
    assert!(
        env.has_file(".my-little-soda"),
        ".my-little-soda directory should be created"
    );
...
async fn example_error_handling() {
    ...
    for fixture_name in &problematic_fixtures {
        ...
        println!(
            "✅ Error handling test passed for problematic fixture: {}",
            fixture_name
        );
    }
...
async fn example_custom_fixture_validation() {
    ...
    assert!(
        fixture.existing_my_little_soda_config.is_some(),
        "Should have existing config"
    );
    assert!(
        fixture.files.contains_key("my-little-soda.toml"),
        "Should contain my-little-soda.toml file"
    );

    let expected_behavior = fixture.expected_init_behavior();
    assert!(
        !expected_behavior.should_succeed_without_force,
        "Should require force flag"
    );
    assert!(
        !expected_behavior.validation_warnings.is_empty(),
    ...
    assert!(
        config_content.contains("old-owner"),
        "Should contain old owner in config"
    );
    assert!(
        config_content.contains("tracing_enabled = false"),
        "Should have existing settings"
    );
...
async fn example_comprehensive_scenario_testing() {
    ...
    println!(
        "✅ Example 8 passed: All {} scenarios completed successfully",
        all_results.len()
    );
}
...
async fn example_advanced_validation() {
    ...
    let cargo_content = env
        .read_file("Cargo.toml")
        .expect("Failed to read Cargo.toml");
    assert!(
        cargo_content.contains("existing-project"),
        "Should have existing project name"
    );

    // Run init
    let result = env
        .run_and_validate_init(2, false, false)
        .await
        .expect("Failed to run init command");
    ...
    assert!(
        env.has_file("src/main.rs"),
        "Original main.rs should be preserved"
    );
    assert!(
        env.has_file("src/lib.rs"),
        "Original lib.rs should be preserved"
    );

    // Custom directory structure check
    let clambake_agents_dir = env.path().join(".my-little-soda/agents");
    assert!(
        clambake_agents_dir.exists(),
        "Agent working directory should be created"
    );
...
async fn comprehensive_fixture_system_integration_test() {
    ...
    for fixture in &all_fixtures {
        ...
        assert!(
            loaded.is_some(),
            "Should be able to load fixture by name: {}",
            fixture.name
        );
    }
    ...
    println!(
        "✅ All {} fixtures integrated successfully with init command",
        successful_integrations
    );
    ...
    println!(
        "✅ Batch processing handled {} fixtures",
        batch_results.results.len()
    );
