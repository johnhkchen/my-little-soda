...
use my_little_soda::{
    ...
    autonomous::{
        ...
        error_recovery::{ErrorType, RecoveryStrategy, FixType, ConfidenceLevel},
    },
...
async fn test_complete_autonomous_workflow() {
    ...
    let mut coordinator = AutonomousCoordinator::new(
        ...
        recovery_client,
        config,
    ).await.expect("Failed to create autonomous coordinator");
    
    // Test assignment
    let assign_result = coordinator.start_autonomous_operation().await;
    assert!(assign_result.is_ok(), "Failed to start autonomous operation: {:?}", assign_result);
    ...
    assert!(stop_result.is_ok(), "Failed to stop autonomous operation: {:?}", stop_result);
...
async fn test_workflow_state_machine_transitions() {
    ...
    assert!(start_work_result.is_ok(), "Start work failed: {:?}", start_work_result);
    ...
    let progress_result = workflow.handle_event(AutonomousEvent::MakeProgress {
        commits: 3,
        files_changed: 5,
    }).await;
    assert!(progress_result.is_ok(), "Make progress failed: {:?}", progress_result);
    ...
    assert!(complete_result.is_ok(), "Complete work failed: {:?}", complete_result);
...
async fn test_blocker_handling_and_recovery() {
    ...
    assert!(blocker_result.is_ok(), "Blocker handling failed: {:?}", blocker_result);
...
async fn test_error_recovery_system() {
    ...
    let recovery_result = error_recovery.execute_recovery_strategy(
        git_error,
        strategy,
        &current_state,
    ).await;
    
    assert!(recovery_result.is_ok(), "Recovery execution failed: {:?}", recovery_result);
...
async fn test_state_persistence() {
    ...
    // Test save
    let save_result = manager.save_state(&persistent_state, CheckpointReason::UserRequested).await;
    ...
    match loaded_state.current_state.unwrap() {
        AutonomousWorkflowState::InProgress { issue, progress, .. } => {
...
async fn test_checkpoint_operations() {
    ...
    // Create checkpoint
    let checkpoint_result = manager.create_checkpoint(&test_state, CheckpointReason::BeforeRecovery).await;
    assert!(checkpoint_result.is_ok(), "Checkpoint creation failed: {:?}", checkpoint_result);
    ...
    assert!(list_result.is_ok(), "Listing checkpoints failed: {:?}", list_result);
    
    let checkpoints = list_result.unwrap();
    assert!(checkpoints.len() > 0, "No checkpoints found");
    
    let found_checkpoint = checkpoints.iter().find(|cp| cp.checkpoint_id == checkpoint_id);
    assert!(found_checkpoint.is_some(), "Created checkpoint not found in list");
    
    // Restore from checkpoint
    let restore_result = manager.restore_from_checkpoint(&test_state.agent_id, &checkpoint_id).await;
    assert!(restore_result.is_ok(), "Checkpoint restoration failed: {:?}", restore_result);
...
async fn test_system_integration() {
    ...
    assert!(integration_result.is_ok(), "Integration system creation failed: {:?}", integration_result);
...
async fn test_merge_conflict_resolution() {
    ...
    let conflict_result = workflow.handle_event(AutonomousEvent::MergeConflictDetected { 
        conflicts 
    }).await;
    
    assert!(conflict_result.is_ok(), "Merge conflict handling failed: {:?}", conflict_result);
    ...
    // Resolve conflicts
    let resolve_result = workflow.handle_event(AutonomousEvent::ConflictsResolved).await;
    assert!(resolve_result.is_ok(), "Conflict resolution failed: {:?}", resolve_result);
...
async fn test_ci_failure_handling() {
    ...
    let failure_result = workflow.handle_event(AutonomousEvent::CIFailureDetected { 
        failures 
    }).await;
    
    assert!(failure_result.is_ok(), "CI failure handling failed: {:?}", failure_result);
...
async fn test_successful_completion_flow() {
    ...
    workflow.handle_event(AutonomousEvent::StartWork).await.unwrap();
    workflow.handle_event(AutonomousEvent::MakeProgress { commits: 5, files_changed: 3 }).await.unwrap();
    workflow.handle_event(AutonomousEvent::CompleteWork).await.unwrap();
    ...
    let merge_result = workflow.handle_event(AutonomousEvent::MergeCompleted { 
        merged_work: completed_work 
    }).await;
    
    assert!(merge_result.is_ok(), "Merge completion failed: {:?}", merge_result);
    ...
    let history = workflow.state_history();
    assert!(history.len() >= 7, "Expected at least 7 state transitions, got {}", history.len());
