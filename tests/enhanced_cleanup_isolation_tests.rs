...
impl ResourceTracker {
    ...
    fn detect_leaks(&self) -> Vec<String> {
        ...
        for dir_metadata in &self.temp_dirs {
            if dir_metadata.path.exists() && dir_metadata.is_critical {
                let age = dir_metadata.created_at.elapsed().unwrap_or_default().as_secs();
                leaks.push(format!(
        ...
        for file_metadata in &self.files_created {
            if file_metadata.path.exists() && file_metadata.is_critical {
                let age = file_metadata.created_at.elapsed().unwrap_or_default().as_secs();
                let size_info = file_metadata.size.map_or("unknown size".to_string(), |s| format!("{}B", s));
                leaks.push(format!(
        ...
        for external_path in &self.external_paths {
            if external_path.exists() {
                leaks.push(format!("Untracked external resource: {}", external_path.display()));
            }
        }
    ...
    fn get_resource_report(&self) -> String {
        ...
        report.push_str(&format!("- Temp directories tracked: {}\n", self.temp_dirs.len()));
        report.push_str(&format!("- Files tracked: {}\n", self.files_created.len()));
        report.push_str(&format!("- Processes tracked: {}\n", self.processes_spawned.len()));
        report.push_str(&format!("- External paths: {}\n", self.external_paths.len()));
        ...
        for dir_metadata in &self.temp_dirs {
            if dir_metadata.path.exists() {
                if let Err(e) = fs::remove_dir_all(&dir_metadata.path) {
                    let severity = if dir_metadata.is_critical { "CRITICAL" } else { "WARNING" };
                    errors.push(format!(
...
impl EnhancedTestHarness {
    ...
    fn verify_isolation(&mut self) -> Result<()> {
        ...
        let marker_file = path.join(format!("marker_{}_{}", std::process::id(), start_time.elapsed().as_nanos()));
        let marker_content = format!("pid:{},thread:{:?},time:{:?}", 
                                      std::process::id(), 
        ...
        if test_file.exists() || binary_file.exists() || nested_file.exists() || marker_file.exists() {
    ...
    fn verify_cross_test_isolation(&self, others: &[&EnhancedTestHarness]) -> Result<()> {
        ...
        for (i, other) in others.iter().enumerate() {
            ...
            if self_path.starts_with(other_path) || other_path.starts_with(self_path) {
                anyhow::bail!("Nested test paths detected with harness {}: self={}, other={}", 
                              i, self_path.display(), other_path.display());
            }
            
            // 3. Verify path distance (should not be adjacent siblings that might interfere)
            if let (Some(self_parent), Some(other_parent)) = (self_path.parent(), other_path.parent()) {
                if self_parent == other_parent {
                    // Same parent directory - verify sufficiently unique names
                    if let (Some(self_name), Some(other_name)) = (self_path.file_name(), other_path.file_name()) {
                        let self_str = self_name.to_string_lossy();
                        let other_str = other_name.to_string_lossy();
                        if self_str.len() < 10 || other_str.len() < 10 {
                            anyhow::bail!("Test directory names too similar/short: {} vs {}", self_str, other_str);
                        }
                    }
                }
            }
            ...
            if other_path.exists() && self_path.exists() {
                ...
                for entry in fs::read_dir(other_path)? {
                    ...
                    for self_entry in &self_files {
                        ...
                        if files_too_similar(&file_name.to_string_lossy(), &self_name.to_string_lossy()) {
                            anyhow::bail!("Similar file names detected: {:?} vs {:?}", file_name, self_name);
                        }
                    }
                }
                ...
                for self_external in &self.resource_tracker.external_paths {
                    for other_external in &other.resource_tracker.external_paths {
                        if self_external == other_external {
                            anyhow::bail!("Shared external resource detected: {}", self_external.display());
                        }
                    }
                }
            }
    ...
    fn get_isolation_report(&self) -> String {
        let mut report = String::new();
        report.push_str(&format!("Isolation Report for: {}\n", self.temp_dir.path().display()));
        report.push_str(&format!("- Isolation verified: {}\n", self.isolation_verified));
        report.push_str(&format!("- Base path: {}\n", self.temp_dir.path().display()));
