...
impl ExistingReadmeTestHarness {
    ...
    async fn run_init(&self, force: bool, dry_run: bool) -> Result<bool> {
        ...
        let fs_ops: Arc<dyn my_little_soda::fs::FileSystemOperations> =
...
async fn test_init_preserves_existing_simple_readme() {
    ...
    assert!(
        success,
        "Init should succeed on repository with existing README"
    );

    // Verify README content is preserved
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Original README content should be preserved"
    );
}
...
"#
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Complex README content should be preserved exactly"
    );
}
...
"#
    ...
    assert!(
        success,
        "Init should succeed with special characters in README"
    );

    // Verify all special characters are preserved
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Special characters in README should be preserved"
    );
}
...
async fn test_init_handles_empty_readme() {
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Empty README should be preserved"
    );
}
...
async fn test_init_handles_very_large_readme() {
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Large README should be preserved completely"
    );
}
...
async fn test_init_graceful_handling_no_data_loss() {
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "README should exist before init"
    );
    assert!(
        !harness.has_clambake_config(),
        "Should not have clambake config before init"
    );
    assert!(
        !harness.has_clambake_directory(),
        "Should not have clambake directory before init"
    );
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "README content must not be lost during init"
    );
...
async fn test_init_conflict_resolution_documentation() {
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "Conflict resolution should preserve existing files"
    );
...
"#
    ...
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "README content must be preserved after init"
    );
...
"#
    ...
    assert!(
        success,
        "C1b: Init command should handle existing files gracefully"
    );

    // C1b Requirement 2: Validate no data loss occurs
    assert!(
        harness.verify_readme_preserved().unwrap(),
        "C1b: No data loss should occur - README must be preserved"
    );
