...
mod tests {
        ...
        pub fn generate_test_environments() -> Vec<CITestMatrix> {
            vec![
                ...
                CITestMatrix {
                    ...
                    expected_optimizations: vec!["caching".to_string(), "parallel_processing".to_string()],
                },
                CITestMatrix {
                ...
                CITestMatrix {
                    ...
                    expected_optimizations: vec!["resource_throttling".to_string(), "backoff_strategies".to_string()],
                },
            ]
        }
    }
    ...
    pub fn property_test_ci_mode_configuration(test_matrix: &CITestMatrix) -> Result<(), String> {
        ...
        assert_eq!(detected_ci_mode, test_matrix.github_actions, 
        ...
        if test_matrix.has_artifacts && test_matrix.github_actions {
            assert!(test_matrix.expected_optimizations.contains(&"caching".to_string()) ||
                   test_matrix.expected_optimizations.contains(&"artifact_compression".to_string()),
                   "Artifact-enabled CI environments should have artifact optimizations");
        }

        // Property 3: Timeout multipliers should be reasonable
        assert!(test_matrix.timeout_multiplier >= 1.0 && test_matrix.timeout_multiplier <= 5.0,
        ...
        if test_matrix.github_actions {
            ...
            let ci_specific_opts = test_matrix.expected_optimizations.iter()
                .filter(|opt| opt.contains("ci_") || opt.contains("artifact_"))
                .count();
            assert!(ci_specific_opts <= 1,
    ...
    fn test_all_environment_configurations() {
        ...
        assert!(failed_tests.is_empty(), 
                ...
                failed_tests);
        assert_eq!(tested_environments, test_environments.len(),
    ...
    fn test_github_actions_environment_consistency() {
        ...
        let test_environments = CITestMatrix::generate_test_environments();
        let github_actions_envs: Vec<_> = test_environments.iter()
        ...
        let total_optimizations: usize = github_actions_envs.iter()
    ...
    fn test_local_environment_characteristics() {
        ...
        let test_environments = CITestMatrix::generate_test_environments();
        let local_envs: Vec<_> = test_environments.iter()
            .filter(|env| !env.github_actions)
            .collect();
        ...
        for env in &local_envs {
            ...
            assert!(env.timeout_multiplier <= 2.0,
            ...
            if env.has_artifacts {
                assert!(env.environment_name.contains("simulation"),
    ...
    fn test_optimization_strategy_validity() {
        ...
        let valid_optimizations = vec![
            "caching", "parallel_processing", "artifact_compression", "api_rate_limiting",
            "minimal_logging", "artifact_simulation", "resource_throttling", "backoff_strategies"
        ];
        ...
        for env in &test_environments {
            for optimization in &env.expected_optimizations {
                ...
                assert!(valid_optimizations.iter().any(|valid| optimization.contains(valid)),
                        "Optimization '{}' should be a valid strategy for environment '{}'",
                        optimization, env.environment_name);
            }
        }
    }
    ...
    fn test_environment_matrix_coverage() {
        ...
        let github_actions_count = test_environments.iter().filter(|e| e.github_actions).count();
        let local_count = test_environments.iter().filter(|e| !e.github_actions).count();
        ...
        assert!(github_actions_count >= 3, "Should test multiple GitHub Actions configurations");
        assert!(local_count >= 1, "Should test local environments");
        assert!(artifact_enabled_count >= 3, "Should test artifact-enabled environments");
        
        // And: We should cover different token strategies
        let token_strategies: std::collections::HashSet<_> = test_environments.iter()
            .map(|e| &e.token_strategy)
            .collect();
        assert!(token_strategies.len() >= 3, "Should cover multiple token strategies");
        
        // And: We should cover different timeout scenarios
        let timeout_ranges = test_environments.iter()
            .map(|e| e.timeout_multiplier)
            .fold((f64::INFINITY, 0.0), |(min, max), val| (min.min(val), max.max(val)));
        assert!(timeout_ranges.1 - timeout_ranges.0 >= 1.0, 
    ...
    fn test_edge_case_environment_handling() {
        ...
        let test_environments = CITestMatrix::generate_test_environments();
        let edge_case_envs: Vec<_> = test_environments.iter()
            .filter(|env| env.environment_name.contains("limited") || 
                         env.environment_name.contains("minimal"))
            .collect();
    ...
    fn test_ci_workflow_execution_patterns() {
        ...
        for env in &test_environments {
            ...
            if env.github_actions {
                ...
                assert!(adjusted_duration <= base_duration * 5,
                        "CI environments should not be excessively slow");
            } else {
                assert!(adjusted_duration <= base_duration * 2,
    ...
    fn test_ci_mode_configuration_under_load() {
        ...
        assert_eq!(successful_executions, concurrent_workflows,
        ...
        for result in results {
            assert!(result.is_ok(), "CI configuration should remain valid under concurrent load");
        }
    }
}