...
impl MockWorkflowCoordinator {
    ...
    pub fn add_issue(&self, issue_number: u64, initial_state: WorkflowState) {
        self.issue_workflows.lock().unwrap().insert(issue_number, initial_state);
    }
    
    pub fn transition_issue(&self, issue_number: u64, trigger: TransitionTrigger) -> Result<WorkflowState, String> {
        let start_time = Instant::now();
        
        let mut workflows = self.issue_workflows.lock().unwrap();
        let current_state = workflows.get(&issue_number)
            .ok_or_else(|| format!("Issue #{} not found", issue_number))?
            .clone();
    ...
    pub fn get_issue_state(&self, issue_number: u64) -> Option<WorkflowState> {
        self.issue_workflows.lock().unwrap().get(&issue_number).cloned()
    }
    ...
    pub fn simulate_clambake_pop(&self) -> Result<Vec<u64>, String> {
        ...
        let available_issues: Vec<u64> = workflows.iter()
    ...
    pub fn simulate_clambake_land(&self, issue_number: u64) -> Result<(), String> {
        let current_state = self.get_issue_state(issue_number)
            .ok_or_else(|| format!("Issue #{} not found", issue_number))?;
        ...
        match current_state {
            ...
            _ => Err(format!("Cannot land issue #{} in state {:?}", issue_number, current_state))
        }
    }
    ...
    pub fn verify_workflow_consistency(&self) -> Result<(), String> {
        ...
        for transition in transitions.iter() {
            if !self.is_valid_transition(&transition.from_state, &transition.to_state, &transition.trigger) {
                return Err(format!(
        ...
        for (issue_num, state) in workflows.iter() {
            match state {
                WorkflowState::Failed(msg) => {
                    ...
                    let has_failure = transitions.iter().any(|t| {
                        matches!(t.trigger, TransitionTrigger::FailureDetected(_))
                    });
                    if !has_failure {
                        return Err(format!("Issue #{} in failed state without failure transition", issue_num));
                    }
                }
                _ => {}
            }
    ...
    fn compute_next_state(&self, current: &WorkflowState, trigger: &TransitionTrigger) -> Result<WorkflowState, String> {
        match (current, trigger) {
            (WorkflowState::Ready, TransitionTrigger::AgentAssignment) => Ok(WorkflowState::Assigned),
            (WorkflowState::Assigned, TransitionTrigger::WorkStarted) => Ok(WorkflowState::Working),
            (WorkflowState::Working, TransitionTrigger::WorkCompleted) => Ok(WorkflowState::Completed),
            (WorkflowState::Completed, TransitionTrigger::LandCommand) => Ok(WorkflowState::ReviewReady),
            (WorkflowState::ReviewReady, TransitionTrigger::CodeRabbitReview) => Ok(WorkflowState::Reviewed),
            (WorkflowState::Reviewed, TransitionTrigger::MergeCommand) => Ok(WorkflowState::MergeReady),
            (WorkflowState::MergeReady, TransitionTrigger::MergeCommand) => Ok(WorkflowState::Merged),
            (_, TransitionTrigger::FailureDetected(msg)) => Ok(WorkflowState::Failed(msg.clone())),
            _ => Err(format!("Invalid transition: {:?} with trigger {:?}", current, trigger))
        }
    }
    
    fn is_valid_transition(&self, from: &WorkflowState, to: &WorkflowState, trigger: &TransitionTrigger) -> bool {
...
mod tests {
    ...
    fn test_bundled_workflow_for_rate_limiting() {
        ...
        assert!(available_work.len() > 1 && available_work.len() <= 3, 
        ...
        for issue_num in &available_work {
            coordinator.transition_issue(*issue_num, TransitionTrigger::AgentAssignment).unwrap();
        }
    ...
    fn test_clambake_land_integration() {
        ...
        coordinator.transition_issue(95, TransitionTrigger::AgentAssignment).unwrap();
        coordinator.transition_issue(95, TransitionTrigger::WorkStarted).unwrap();
        ...
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::ReviewReady);
        
        // And: Transitions should include both completion and land
        let transitions = coordinator.get_transitions();
        let completion_transitions: Vec<_> = transitions.iter()
            .filter(|t| matches!(t.trigger, TransitionTrigger::WorkCompleted | TransitionTrigger::LandCommand))
            .collect();
        assert_eq!(completion_transitions.len(), 2, "Should have both completion and land transitions");
    }
    ...
    fn test_invalid_workflow_transitions() {
        ...
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::Ready);
    }
    ...
    fn test_workflow_failure_handling() {
        ...
        let verification = coordinator.verify_workflow_consistency();
        assert!(verification.is_ok(), "Failed workflow should still be consistent");
    }
    ...
    fn test_multiple_issues_different_workflow_phases() {
        ...
        // Issue 96: Assignment
        coordinator.transition_issue(96, TransitionTrigger::AgentAssignment).unwrap();
        
        // Issue 97: Working
        coordinator.transition_issue(97, TransitionTrigger::AgentAssignment).unwrap();
        coordinator.transition_issue(97, TransitionTrigger::WorkStarted).unwrap();
        
        // Issue 98: Completed and ready for review
        coordinator.transition_issue(98, TransitionTrigger::AgentAssignment).unwrap();
        coordinator.transition_issue(98, TransitionTrigger::WorkStarted).unwrap();
        coordinator.transition_issue(98, TransitionTrigger::WorkCompleted).unwrap();
        coordinator.transition_issue(98, TransitionTrigger::LandCommand).unwrap();
        
        // Then: Each issue should be in the correct state
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::Ready);
        assert_eq!(coordinator.get_issue_state(96).unwrap(), WorkflowState::Assigned);
        assert_eq!(coordinator.get_issue_state(97).unwrap(), WorkflowState::Working);
        assert_eq!(coordinator.get_issue_state(98).unwrap(), WorkflowState::ReviewReady);
        
        // And: clambake pop should only return ready issues
        let available_work = coordinator.simulate_clambake_pop().unwrap();
        assert_eq!(available_work, vec![95], "Only issue 95 should be available for assignment");
        
        // And: Workflow should be consistent across all issues
        let verification = coordinator.verify_workflow_consistency();
        assert!(verification.is_ok(), "Multi-issue workflow should be consistent");
    }
    ...
    fn test_coderabbit_review_integration() {
        ...
        coordinator.transition_issue(95, TransitionTrigger::AgentAssignment).unwrap();
        coordinator.transition_issue(95, TransitionTrigger::WorkStarted).unwrap();
        coordinator.transition_issue(95, TransitionTrigger::WorkCompleted).unwrap();
        coordinator.transition_issue(95, TransitionTrigger::LandCommand).unwrap();
        ...
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::Reviewed);
        ...
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::MergeReady);
        
        // Final merge
        let final_merge = coordinator.transition_issue(95, TransitionTrigger::MergeCommand);
        assert!(final_merge.is_ok());
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::Merged);
    }
    ...
    fn test_state_machine_compatibility() {
        ...
        // Valid sequence
        if coordinator.transition_issue(95, TransitionTrigger::AgentAssignment).is_ok() {
            successful_transitions += 1;
        } else {
            failed_transitions += 1;
        }
        
        if coordinator.transition_issue(95, TransitionTrigger::WorkStarted).is_ok() {
            successful_transitions += 1;
        } else {
            failed_transitions += 1;
        }
        ...
        if coordinator.transition_issue(95, TransitionTrigger::MergeCommand).is_err() {
            // This is expected - counting as successful validation
            successful_transitions += 1;
        }
        
        // Continue valid sequence
        if coordinator.transition_issue(95, TransitionTrigger::WorkCompleted).is_ok() {
            successful_transitions += 1;
        } else {
            failed_transitions += 1;
        }
        ...
        assert!(successful_transitions >= 4, "Should have successful state validation");
        
        // And: Final state should reflect valid transitions only
        assert_eq!(coordinator.get_issue_state(95).unwrap(), WorkflowState::Completed);
    }
}