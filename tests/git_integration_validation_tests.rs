...
fn test_git_initialization_validation() {
    ...
    assert_eq!(String::from_utf8_lossy(&config_output.stdout).trim(), "Test User");
    ...
    assert_eq!(String::from_utf8_lossy(&email_output.stdout).trim(), "test@example.com");
}
...
fn test_git_commit_with_validation() {
    ...
    assert_eq!(String::from_utf8_lossy(&log_output.stdout).trim(), "Test commit for validation");
...
fn test_git_remote_validation() {
    ...
    assert!(remotes_text.contains("upstream\thttps://github.com/upstream-owner/upstream-repo.git (fetch)"));
    assert!(remotes_text.contains("upstream\thttps://github.com/upstream-owner/upstream-repo.git (push)"));
...
fn test_git_branch_operations() {
    ...
    let current_branch = Command::new("git")
        ...
        .output()
        .unwrap();
    assert_eq!(String::from_utf8_lossy(&current_branch.stdout).trim(), "feature-branch");
}
...
fn test_git_status_comprehensive_validation() {
    ...
    harness.create_file("untracked.txt", "Untracked file").unwrap();
...
fn test_git_clean_operations() {
    ...
    harness.create_file("untracked1.txt", "Untracked 1").unwrap();
    harness.create_file("untracked2.tmp", "Untracked 2").unwrap();
