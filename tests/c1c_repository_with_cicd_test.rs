...
async fn test_init_preserves_existing_cicd_workflows() {
    ...
    assert!(
        env.has_file(".github/workflows/ci.yml"),
        "Should have CI workflow"
    );
    assert!(
        env.has_file(".github/workflows/release.yml"),
        "Should have release workflow"
    );
    assert!(
        env.has_file(".github/workflows/security.yml"),
        "Should have security workflow"
    );
    assert!(env.has_file("Dockerfile"), "Should have Docker setup");
    assert!(
        env.has_file("docker-compose.yml"),
        "Should have Docker Compose setup"
    );
    assert!(
        env.has_file("codecov.yml"),
        "Should have code coverage config"
    );
    assert!(
        env.has_file(".pre-commit-config.yaml"),
    ...
    assert!(
        ci_workflow.contains("cargo test --verbose"),
        "CI should run tests"
    );
    assert!(
        ci_workflow.contains("cargo clippy -- -D warnings"),
        "CI should run clippy"
    );
    assert!(
    ...
    assert!(
        result.success,
        "Init should succeed on repository with CI/CD: {:?}",
    ...
    assert!(
        env.has_file(".github/workflows/ci.yml"),
        "CI workflow should be preserved"
    );
    assert!(
        env.has_file(".github/workflows/release.yml"),
        "Release workflow should be preserved"
    );
    assert!(
        env.has_file(".github/workflows/security.yml"),
        "Security workflow should be preserved"
    );
    assert!(
        env.has_file("Dockerfile"),
        "Docker setup should be preserved"
    );
    assert!(
        env.has_file("docker-compose.yml"),
        "Docker Compose should be preserved"
    );

    // Verify CI workflow content is unchanged
    let post_init_ci_workflow = env
        .read_file(".github/workflows/ci.yml")
        .expect("Failed to read CI workflow after init");
    assert_eq!(
        ci_workflow, post_init_ci_workflow,
        "CI workflow should be unchanged"
    );
}
...
async fn test_init_enhances_existing_cicd_setup() {
    ...
    let result = env
        .run_and_validate_init(1, false, false)
        .await
        .expect("Failed to run init command");
    ...
    let post_init = env
        .validate_post_init_state(false)
        .expect("Failed to validate post-init state");
    assert!(
        post_init.all_expectations_met(),
        "Post-init validation should pass"
    ...
    assert!(
        ...
        "Clambake config should be created"
    );
    assert!(
        env.has_file(".my-little-soda"),
        "Clambake directory should be created"
    );

    // Verify Docker Compose has expected services that won't conflict
    let docker_compose = env
        .read_file("docker-compose.yml")
        .expect("Failed to read docker-compose.yml");
    assert!(
        docker_compose.contains("postgres:"),
        "Should preserve postgres service"
    );
    assert!(
        docker_compose.contains("redis:"),
        "Should preserve redis service"
    );
}
...
async fn test_workflow_preservation_comprehensive() {
    ...
    for (scenario_name, agents, force, dry_run) in scenarios {
        let env =
            InitCommandTestEnvironment::from_fixture_name("repository_with_existing_cicd_files")
                .expect("Failed to create CI/CD test environment");
        ...
        let result = env
            .run_and_validate_init(agents, force, dry_run)
            .await
            .expect(&format!(
...
fn test_cicd_integration_behavior_specification() {
    ...
    assert!(
        ...
        "Clean CI/CD repo should allow init"
    );
    assert!(
    ...
    for expected_file in &expected_cicd_files {
        assert!(
            ...
            "Fixture should contain CI/CD file: {}",
            expected_file
        );
    }
    ...
    assert!(
        ci_workflow.contains("matrix:"),
        "CI should use build matrix"
    );
    assert!(
        ci_workflow.contains("rust:"),
    ...
    assert!(
        ci_workflow.contains("nightly"),
        "CI should test nightly Rust"
    );

    // Verify security workflow automation
    let security_workflow = fixture.files.get(".github/workflows/security.yml").unwrap();
    assert!(
        security_workflow.contains("security audit"),
        "Should have security auditing"
    );
    assert!(
...
async fn test_no_automation_conflicts() {
    ...
    assert!(
        env.has_file("my-little-soda.toml"),
        "Clambake config created"
    );
    assert!(
        env.has_file(".my-little-soda"),
        "Clambake directory created"
    );

    // Verify existing CI/CD automation is preserved
    assert!(
        env.has_file(".github/workflows/ci.yml"),
        "CI workflow preserved"
    );
    assert!(
        env.has_file(".pre-commit-config.yaml"),
    ...
    assert!(
        !env.has_file(".my-little-soda/workflows"),
        "No workflow directory conflict"
    );
    assert!(
        !env.has_file("clambake.yml"),
        "No YAML config naming conflict"
    );
    ...
    assert!(
        ci_content.contains("name: CI"),
        "CI workflow name preserved"
    );
    assert!(
        ci_content.contains("cargo test --verbose"),
        "CI test command preserved"
    );
    assert!(ci_content.contains("cargo clippy"), "CI linting preserved");

    // Verify pre-commit config doesn't conflict with clambake
    let precommit_content = env
        .read_file(".pre-commit-config.yaml")
        .expect("Failed to read pre-commit config");
    assert!(
        precommit_content.contains("cargo fmt"),
        "Pre-commit formatting preserved"
    );
    assert!(
        precommit_content.contains("cargo clippy"),
        "Pre-commit linting preserved"
    );
}
...
async fn test_complete_c1c_scenario() {
    ...
    for file in &cicd_files {
        assert!(
            env.has_file(file),
            "CI/CD file {} should be preserved",
            file
        );
    }
    ...
    let ci_workflow = env
        ...
        .expect("Failed to read CI workflow");
    assert!(
        ci_workflow.contains("name: CI"),
        "CI workflow should maintain structure"
    );
