...
impl MockAgentCoordinator {
    ...
    pub fn add_agent(&self, agent_id: &str) {
        self.agents
            .lock()
            .unwrap()
            .insert(agent_id.to_string(), AgentState::Available);
    }
    ...
    pub fn add_issue(&self, issue_number: u64, status: IssueStatus) {
        ...
        self.issue_states
            .lock()
            .unwrap()
            .insert(issue_number, issue_state);
    }
    ...
    pub fn simulate_clambake_pop(&self, agent_id: &str) -> Result<Option<u64>, String> {
        ...
        let assignable_issues: Vec<&IssueState> = issues
            .values()
            .filter(|issue| issue.status == IssueStatus::Ready && issue.assigned_agent.is_none())
            .collect();
...
mod tests {
    ...
    fn test_complete_agent_lifecycle_success() {
        ...
        assert!(issue_state
            .labels
            .contains(&"route:ready_to_merge".to_string()));
        ...
        assert!(
            verification.is_ok(),
            "No reassignment regression should be detected"
        );
    }
    ...
    fn test_completed_work_never_reassigned() {
        ...
        assert!(
            available_work.is_none(),
            "Completed issue should not be available for reassignment"
        );
        ...
        assert!(assignment_result
            .unwrap_err()
            .contains("Cannot assign completed issue"));
    ...
    fn test_multiple_agents_concurrent_lifecycle() {
        ...
        assert_eq!(
            coordinator.get_agent_state("agent001").unwrap(),
            AgentState::Available
        );
        assert_eq!(
            coordinator.get_agent_state("agent002").unwrap(),
            AgentState::Working(96)
        );
        assert_eq!(
            coordinator.get_agent_state("agent003").unwrap(),
            AgentState::Available
        );

        // And: Issues should have correct states
        assert_eq!(
            coordinator.get_issue_state(95).unwrap().status,
            IssueStatus::Completed
        );
        assert_eq!(
            coordinator.get_issue_state(96).unwrap().status,
            IssueStatus::InProgress
        );
        assert_eq!(
            coordinator.get_issue_state(97).unwrap().status,
            IssueStatus::Completed
        );
        ...
        assert!(
            work_for_agent001.is_none(),
            "No completed work should be available"
        );
        assert!(
            work_for_agent003.is_none(),
            "No completed work should be available"
        );
        ...
        assert!(
            verification.is_ok(),
            "No reassignment regressions in concurrent scenario"
        );
    }
    ...
    fn test_agent_state_consistency_during_transitions() {
        ...
        assert!(matches!(
            events[1].event_type,
            LifecycleEventType::StartedWork
        ));
        assert!(matches!(
            events[2].event_type,
            LifecycleEventType::CompletedWork
    ...
    fn test_edge_case_partial_state_transitions() {
        ...
        assert!(
            ...
            "Cannot assign working agent to new issue"
        );
    ...
    fn test_clambake_pop_respects_agent_availability() {
        ...
        assert!(
            work_assigned.is_none(),
            "Assigned agent should not get new work"
        );
        assert!(
            work_working.is_none(),
            "Working agent should not get new work"
        );
        assert!(
            work_completed.is_none(),
            "Completed agent should not get new work"
        );
    }
}
