...
use super::repository_states::{
    InitBehaviorExpectation, RepositoryFixtureLoader, RepositoryStateFixture,
};
use anyhow::Result;
...
impl InitCommandTestEnvironment {
    ...
    pub async fn run_and_validate_init(
        &self,
        agents: u32,
        force: bool,
        dry_run: bool,
        ...
        let test_result = InitCommandTestResult {
            ...
            expected_success: if force {
                true
            } else {
                self.expected_behavior.should_succeed_without_force
            },
        };
    ...
    pub fn validate_post_init_state(&self, dry_run: bool) -> Result<PostInitValidationResult> {
        ...
        validation.expected_config_creation =
            self.expected_behavior.should_create_config && !dry_run;
        ...
        validation.expected_directory_creation =
            self.expected_behavior.should_create_directories && !dry_run;

        // Validate expectations match reality
        validation.config_expectation_met =
            validation.config_created == validation.expected_config_creation;
        validation.directory_expectation_met =
            validation.directories_created == validation.expected_directory_creation;
    ...
    pub fn read_file(&self, relative_path: &str) -> Result<String> {
        ...
        std::fs::read_to_string(file_path)
            .map_err(|e| anyhow::anyhow!("Failed to read {}: {}", relative_path, e))
    }
}
...
impl InitCommandBatchTester {
    ...
    pub async fn test_all_fixtures(
        agents: u32,
        force: bool,
        dry_run: bool,
    ...
    pub fn failure_summaries(&self) -> Vec<(String, String)> {
        self.results
            ...
            .filter_map(|result| {
                result
                    .failure_summary()
                    .map(|summary| (result.fixture_name.clone(), summary))
            })
            .collect()
    }
}
...
pub mod assertions {
    ...
    pub fn assert_init_succeeds_for_clean_repo(result: &InitCommandTestResult, fixture_name: &str) {
        assert!(
            ...
            fixture_name, result.error_message
        );
    }
    ...
    pub fn assert_post_init_validation_passes(
        validation: &PostInitValidationResult,
        fixture_name: &str,
    ) {
        assert!(
