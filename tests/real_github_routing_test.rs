...
mod real_github_routing_tests {
    ...
    async fn test_should_fetch_real_issues_from_github_not_mock() {
        ...
        for issue in issues.iter().take(3) {
            ...
            assert!(
                ...
                "Should have real GitHub issue title"
            );
            assert!(
                issue
                    .html_url
    ...
    async fn test_should_filter_issues_by_route_ready_label() {
        ...
        let routable_issues: Vec<_> = all_issues
            .into_iter()
            .filter(|issue| issue.labels.iter().any(|label| label.name == "route:ready"))
            .collect();

        println!(
            "🏷️  Issues with route:ready label: {}",
            routable_issues.len()
        );
    ...
    async fn test_should_actually_assign_issues_in_github() {
        ...
        for assignment in assignments.iter().take(2) {
            println!(
                "🤖 Testing assignment: issue #{} → agent {}",
            ...
            if current_issue.assignee.is_some() {
                println!(
                    "  ✅ Issue #{} is actually assigned in GitHub!",
                    assignment.issue.number
                );
            } else {
                println!(
                    "  ❌ EXPECTED FAILURE: Issue #{} not assigned in GitHub",
                    assignment.issue.number
                );
                println!(
                    "     📝 route command showed assignment but didn't actually assign in GitHub"
                );
            }
        }

        println!(
            "💡 This test drives implementing REAL GitHub assignment, not just console output"
        );
    }
    ...
    async fn test_should_create_branches_for_assigned_work() {
        ...
        for assignment in assignments.iter().take(1) {
            let expected_branch = format!(
                "agent-{}/{}",
                assignment.assigned_agent.id, assignment.issue.number
            );
        ...
        let high_priority_issues: Vec<_> = all_issues
            ...
            .filter(|issue| {
                issue
                    .labels
                    .iter()
                    .any(|label| label.name == "route:priority-high")
            })
            .collect();
    ...
    async fn test_route_command_shows_real_github_data_not_mock() {
        ...
        println!(
            "🎯 Route operation returned {} assignments",
            assignments.len()
        );
        ...
        for (i, assignment) in assignments.iter().take(2).enumerate() {
            ...
            assert!(
                assignment.issue.number > 0,
                "Should have real issue number, not mock"
            );
            assert!(
                !assignment.issue.title.contains("Generated ticket"),
        ...
        if assignments.is_empty() {
            ...
            println!(
                "   💡 Create with: gh issue create --title 'Test work' --label 'route:ready'"
            );
        } else {
    ...
    async fn test_pop_command_returns_single_task() {
        ...
        let router = AgentRouter::new().await.expect("Failed to create router");
        ...
        match popped_task {
            Ok(Some(task)) => {
                ...
                println!(
                    "  🌿 Branch: {}/{}",
                    task.assigned_agent.id, task.issue.number
                );
                ...
                if let Ok(Some(second_task)) = second_pop {
                    assert_ne!(
                        second_task.issue.number, task.issue.number,
    ...
    async fn test_default_run_explains_how_to_get_work() {
        ...
        assert!(
            true,
            "This drives TDD implementation of new default behavior"
        );
    }
}
