...
use fixtures::automated_validators::{
    create_standard_init_expectations, ContentExpectation, ContentValidator, FileSystemValidator,
    GitConfigExpectations, GitConfigValidator, ValidationResultReporter,
};
use fixtures::init_integration::{
    InitCommandBatchTester, InitCommandTestEnvironment, TestScenario,
};
...
async fn test_comprehensive_automated_validation_system() {
    ...
    println!(
        "📋 Testing {} fixtures across multiple scenarios",
        fixtures.len()
    );
    ...
    for scenario in scenarios {
        ...
        let batch_results = InitCommandBatchTester::test_scenario_across_fixtures(scenario.clone())
            .await
            .expect("Failed to run batch tests");

        println!(
            "   Init command results: {}/{} fixtures passed",
            batch_results.passed_count(),
        ...
        for fixture_result in &batch_results.results {
            let validation_report =
                validate_single_fixture_result(&fixture_result.fixture_name, &scenario)
                    .await
    ...
    let failure_analysis =
        ValidationResultReporter::generate_failure_analysis(&all_validation_reports);
    ...
    if !failure_analysis.failed_validations.is_empty() {
        ...
        for failed in &failure_analysis.failed_validations {
            println!(
                "   {} ({}): {} errors",
                failed.fixture, failed.scenario, failed.error_count
            );
        }
    }
    ...
    assert!(
        failure_analysis.success_rate >= 0.7,
...
async fn test_file_existence_validation_detailed() {
    ...
    println!(
        "Directories missing: {}",
        fs_report.directories_missing.len()
    );
    ...
    assert!(
        !fs_report.files_missing.is_empty() || !fs_report.directories_missing.is_empty(),
...
async fn test_content_validation_detailed() {
    ...
    let content_report =
        ContentValidator::validate_file_contents(env.path(), &content_expectations)
    ...
    assert!(
        content_report.success,
        "Content validation should pass for test fixture"
    );
...
async fn test_git_configuration_validation_detailed() {
    ...
    let git_report = GitConfigValidator::validate_git_configuration(env.path(), &git_expectations)
    ...
    println!(
        ...
        git_report.working_directory_clean
    );
    ...
    assert!(
        git_report.git_repo_exists,
        "Test fixture should have git repository"
    );
...
async fn test_validation_result_reporting_detailed() {
    ...
    println!(
        ...
        summary.content_validation_passed
    );
    ...
    assert!(
        ...
        "Overall success should be false due to content errors"
    );
    assert!(
        summary.filesystem_passed,
        "Filesystem validation should pass"
    );
    assert!(
        !summary.content_validation_passed,
        "Content validation should fail"
    );
    assert!(summary.git_validation_passed, "Git validation should pass");
    assert_eq!(
        summary.total_errors, 1,
        "Should have exactly 1 error from content validation"
    );
...
async fn test_cross_scenario_validation_consistency() {
    ...
    let scenarios = vec![TestScenario::dry_run(), TestScenario::force_init()];

    let mut scenario_reports = Vec::new();

    for scenario in scenarios {
        let report = validate_single_fixture_result(fixture_name, &scenario)
            .await
            .expect("Validation failed");
...
async fn validate_single_fixture_result(
    ...
    let _init_result = env
        .run_and_validate_init(scenario.agents, scenario.force, scenario.dry_run)
        .await?;
...
async fn test_validation_error_reporting_clarity() -> anyhow::Result<()> {
    ...
    assert!(
        error_text.contains("Required pattern not found"),
        "Should report missing required pattern"
    );
    assert!(
        error_text.contains("Forbidden pattern found"),
        "Should report forbidden pattern"
    );
    assert!(
        error_text.contains("Regex pattern not matched"),
        "Should report unmatched regex"
    );
    assert!(
        error_text.contains("lines"),
        "Should report line count issue"
    );
...
async fn test_standard_init_expectations_coverage() {
    ...
    println!(
        "Git expectations: should_be_git_repo = {}",
        git_expectations.should_be_git_repo
    );

    // Verify standard expectations cover key init command outputs
    assert!(
        expected_files.contains(&"my-little-soda.toml".to_string()),
        "Should expect my-little-soda.toml file"
    );
    assert!(
        expected_directories.contains(&".my-little-soda".to_string()),
        "Should expect .my-little-soda directory"
    );
    assert!(
        expected_directories.contains(&".my-little-soda/credentials".to_string()),
        "Should expect .my-little-soda/credentials directory"
    );
    assert!(
        expected_directories.contains(&".my-little-soda/agents".to_string()),
        "Should expect .my-little-soda/agents directory"
    );

    // Verify content expectations for my-little-soda.toml
    let config_expectation = content_expectations
        .get("my-little-soda.toml")
