...
async fn test_workflow_state_transition_performance() {
    ...
    println!("Testing workflow state transition performance over {} iterations", iterations);
    
    for i in 0..iterations {
        let start_time = Instant::now();
        
        // Reset workflow for each iteration
        let assign_result = workflow.handle_event(AutonomousEvent::AssignAgent {
        ...
        if i % 10 == 0 {
            println!("Completed {} iterations, avg duration: {:?}", i, metrics.average_duration());
        }
    }
    ...
    assert!(metrics.success_rate() > 0.95, "Success rate too low: {:.2}%", metrics.success_rate() * 100.0);
    assert!(metrics.average_duration() < Duration::from_millis(50), 
           "Average operation too slow: {:?}", metrics.average_duration());
    assert!(metrics.max_duration() < Duration::from_millis(500),
           "Max operation too slow: {:?}", metrics.max_duration());
}
...
async fn test_drift_detection_performance() {
    ...
    println!("Testing drift detection performance over {} iterations", iterations);
    ...
    for i in 0..iterations {
        ...
        if i % 10 == 0 {
            println!("Completed {} state updates, avg duration: {:?}", i, metrics.average_duration());
        }
    }
    ...
    println!("  Average update duration: {:?}", metrics.average_duration());
    println!("  Update success rate: {:.2}%", metrics.success_rate() * 100.0);
    println!("  Validation duration: {:?}", validation_duration);
    println!("  Validation result: {:?}", validation_result.is_ok());
    
    // Performance assertions
    assert!(metrics.success_rate() > 0.95, "State update success rate too low");
    assert!(metrics.average_duration() < Duration::from_millis(10),
           "State updates too slow: {:?}", metrics.average_duration());
    assert!(validation_duration < Duration::from_secs(30),
           "State validation too slow: {:?}", validation_duration);
}
...
async fn test_memory_usage_during_extended_operation() {
    ...
    println!("  Memory growth: {} KB ({:.2}%)", memory_growth / 1024, growth_percentage);
    
    // Memory growth should be reasonable (less than 50% increase)
    assert!(growth_percentage < 50.0, 
           "Memory growth too high: {:.2}% ({}KB -> {}KB)", 
           growth_percentage, initial_memory / 1024, final_memory / 1024);
}
...
async fn test_concurrent_workflow_performance() {
    ...
    println!("Testing {} concurrent workflows with {} operations each", 
        ...
        let task = tokio::spawn(async move {
            ...
            for op_id in 0..OPERATIONS_PER_WORKFLOW {
                ...
                let event = match op_id % 4 {
                    ...
                    2 => AutonomousEvent::MakeProgress { commits: 1, files_changed: 2 },
    ...
    println!("  Average operation time: {:?}", final_metrics.average_duration());
    println!("  Success rate: {:.2}%", final_metrics.success_rate() * 100.0);
    
    // Performance assertions
    assert!(final_metrics.success_rate() > 0.90, "Concurrent success rate too low");
    assert!(final_metrics.average_duration() < Duration::from_millis(100),
           "Concurrent operations too slow");
    assert_eq!(final_metrics.operation_count, (CONCURRENT_WORKFLOWS * OPERATIONS_PER_WORKFLOW) as u64);
}
...
async fn test_large_state_history_performance() {
    ...
    println!("Testing large state history performance with {} transitions", state_transitions);
    ...
    for i in 0..state_transitions {
        ...
        let result = workflow.handle_event(AutonomousEvent::AssignAgent {
            agent: AgentId(agent_id),
            workspace_ready: true,
        }).await;
        
        assert!(result.is_ok(), "State transition {} failed: {:?}", i, result);
    ...
    println!("  Status transitions count: {}", status_report.transitions_count);
    
    // Performance assertions
    assert_eq!(history.len(), state_transitions as usize);
    assert_eq!(status_report.transitions_count, state_transitions as u64);
    assert!(history_access_time < Duration::from_millis(10), 
           "History access too slow: {:?}", history_access_time);
    assert!(status_time < Duration::from_millis(50),
           "Status generation too slow with large history: {:?}", status_time);
    assert!(history_build_time < Duration::from_secs(5),
           "History building too slow: {:?}", history_build_time);
}
...
async fn test_rapid_event_processing_performance() {
    ...
    println!("Testing rapid event processing with {} events", rapid_events);
    ...
    for i in 0..rapid_events {
        let result = workflow.handle_event(AutonomousEvent::MakeProgress {
            commits: 1,
            files_changed: (i % 5) + 1,
        }).await;
    ...
    // Performance assertions
    assert!(successful_events > rapid_events * 9 / 10, "Too many failed events");
    assert!(events_per_second > 100.0, "Event processing too slow: {:.2} events/sec", events_per_second);
    assert!(average_event_time < Duration::from_millis(10),
           "Average event time too slow: {:?}", average_event_time);
}
...
async fn test_drift_detection_interval_performance() {
    ...
    for interval_minutes in test_intervals {
        println!("Testing drift detection with {}-minute intervals", interval_minutes);
        ...
        let detector = StateDriftDetector::new(github_client.clone(), 
        ...
        println!("  Interval {}min - Active needs validation: {}, Idle needs validation: {}", 
        ...
        assert!(report_time < Duration::from_millis(10), 
               "Report generation too slow for interval {}: {:?}", interval_minutes, report_time);
    }
}
...
async fn test_performance_regression_baseline() {
    ...
    assert!(baseline_metrics["workflow_creation"] < Duration::from_millis(10),
           "Workflow creation baseline too slow");
    assert!(baseline_metrics["github_client_creation"] < Duration::from_millis(100),
           "GitHub client creation baseline too slow");  
    assert!(baseline_metrics["drift_detector_creation"] < Duration::from_millis(50),
