...
impl ExpectedFilesAndDirectories {
    fn standard_init_expectations() -> Self {
        Self {
            required_files: vec!["my-little-soda.toml".to_string()],
...
impl FileSystemValidator {
    ...
    fn validate_file_permissions(file_path: &Path) -> Result<()> {
        ...
        if metadata.permissions().readonly()
            && file_path.file_name().unwrap() != "my-little-soda.toml"
        {
            return Err(anyhow::anyhow!(
                "File is readonly when it should be writable"
            ));
        }
    ...
    fn validate_directory_permissions(dir_path: &Path) -> Result<()> {
        ...
        if metadata.permissions().readonly() {
            return Err(anyhow::anyhow!(
                "Directory is readonly when it should be writable"
            ));
        }
    ...
    fn validate_directory_hierarchy(repo_path: &Path, report: &mut ValidationReport) -> Result<()> {
        ...
        if credentials_dir.exists() && credentials_dir.parent() != Some(&clambake_dir) {
            report.add_error(
                "credentials directory not properly nested under .my-little-soda".to_string(),
            );
        }
        ...
        if agents_dir.exists() && agents_dir.parent() != Some(&clambake_dir) {
            report.add_error(
                "agents directory not properly nested under .my-little-soda".to_string(),
            );
        }
...
async fn test_init_creates_all_required_files_dry_run() {
    ...
    let binary_path = std::env::current_dir()
        .unwrap()
        .join("target/debug/my-little-soda");
    ...
    assert!(
        ...
        "Init command should succeed in dry run"
    );
    ...
    assert!(
        ...
        "Dry run validation should pass: {:?}",
        validation_report
    );
    assert!(
        validation_report.files_missing.is_empty(),
        "No files should be missing in dry run validation"
    );
    assert!(
        validation_report.directories_missing.is_empty(),
        "No directories should be missing in dry run validation"
    );
}
...
async fn test_init_creates_all_required_files_real_run() {
    ...
    if !output.status.success() {
        println!(
            "Init command failed (likely due to missing GitHub authentication): {}",
    ...
    assert!(
        validation_report.success,
        "Real run validation should pass: {:?}",
        validation_report
    );
    assert!(
        validation_report.files_missing.is_empty(),
        "All required files should be created: missing {:?}",
        validation_report.files_missing
    );
    assert!(
        validation_report.directories_missing.is_empty(),
        "All required directories should be created: missing {:?}",
        validation_report.directories_missing
    );
    ...
    assert!(
        ...
        "Config file validation should pass: {:?}",
        config_validation
    );
}
...
async fn test_init_directory_structure_validation() {
    ...
    fs::write(
        repo_path.join("my-little-soda.toml"),
        r#"
[github]
owner = "test"
repo = "test"
...
"#,
    ...
    assert!(
        validation_report.structure_issues.is_empty(),
        "No structure issues should be found: {:?}",
        validation_report.structure_issues
    );
}
...
"#,
    ...
    assert!(
        !config_validation.success,
        "Config validation should fail with missing sections"
    );
    assert!(
        config_validation.structure_issues.len() >= 3,
        "Should detect multiple missing sections: {:?}",
        config_validation.structure_issues
    );
}
...
async fn test_permission_validation() {
    ...
    if !validation_report.success {
        println!(
            ...
            validation_report.permission_issues
        );
    }
}
...
async fn test_comprehensive_file_directory_validation_checklist() {
    ...
    assert!(
        !expectations.required_files.is_empty(),
        "Should have required files"
    );
    assert!(
        !expectations.required_directories.is_empty(),
    ...
    assert!(
        expectations
            .required_files
            .contains(&"my-little-soda.toml".to_string()),
        "Should expect my-little-soda.toml file"
    );
    assert!(
        expectations
            .required_directories
            .contains(&".my-little-soda".to_string()),
        "Should expect .my-little-soda directory"
    );
    assert!(
        expectations
            .required_directories
            .contains(&".my-little-soda/credentials".to_string()),
        "Should expect .my-little-soda/credentials directory"
    );
    assert!(
        expectations
            .required_directories
            .contains(&".my-little-soda/agents".to_string()),
    ...
    println!(
        ...
        expectations.on_demand_directories
    );
}
