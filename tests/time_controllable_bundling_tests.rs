...
mod tests {
    ...
    async fn test_bundling_schedule_timing_logic() {
        ...
        assert!(schedule.next_departure > Local::now(), "Next departure should be in the future");
        
        // Minutes until departure should be reasonable (0-10 minutes)
        assert!(schedule.minutes_until_departure >= 0, "Minutes until departure should be non-negative");
        assert!(schedule.minutes_until_departure <= 10, "Minutes until departure should be at most 10");
        
        // Status should match the minutes until departure
        match schedule.status {
            ScheduleStatus::Waiting => {
                assert!(schedule.minutes_until_departure > 3, "Waiting status should have > 3 minutes");
            }
            ScheduleStatus::Boarding => {
                assert!(schedule.minutes_until_departure <= 3 && schedule.minutes_until_departure > 0, 
                       "Boarding status should have 1-3 minutes");
            }
            ScheduleStatus::Departing => {
                assert!(schedule.minutes_until_departure <= 0, "Departing status should have <= 0 minutes");
            }
        }
    }
    ...
    async fn test_bundle_window_deterministic_naming() {
        ...
        assert_eq!(branch_name, branch_name_2, "Bundle names should be deterministic regardless of input order");
        ...
        assert!(timestamp_part.contains("bundle/"), "Should contain bundle prefix");
    }
    ...
    async fn test_bundle_window_consistency() {
        ...
        assert!(time_diff < 60, "Bundle windows should be consistent within short time period");
        ...
        assert_ne!(branch1, branch2, "Different issues should produce different branch names");
        assert!(branch1.contains("issues_100_200"), "Branch should contain first issue set");
        assert!(branch2.contains("issues_300_400"), "Branch should contain second issue set");
    }
    ...
    async fn test_departure_time_detection_logic() {
        ...
        assert_eq!(is_departure_1, expected_departure, 
        ...
        assert_eq!(is_departure_2, expected_departure_2,
    ...
    async fn test_schedule_status_transitions_logic() {
        ...
        match schedule.status {
            ScheduleStatus::Waiting => {
                assert!(schedule.minutes_until_departure > 3, 
            ...
            ScheduleStatus::Departing => {
                assert!(schedule.minutes_until_departure <= 0,
        ...
        if !matches!(schedule.status, ScheduleStatus::Departing) {
            assert!(schedule.next_departure > Local::now(), 
    ...
    async fn test_bundle_name_uniqueness() {
        ...
        for i in 0..10 {
            ...
            assert!(bundle_names.insert(bundle_name.clone()), 
                   "Bundle name should be unique: {}", bundle_name);
            
            // Name should contain the expected issues
            assert!(bundle_name.contains(&format!("issues_{}_{}", 100 + i, 200 + i)),
                   "Bundle name should contain issue numbers: {}", bundle_name);
        }
        
        // Should have 10 unique bundle names
        assert_eq!(bundle_names.len(), 10, "Should generate 10 unique bundle names");
        
        // All names should start with bundle prefix
        for name in &bundle_names {
            assert!(name.starts_with("bundle/"), "All names should start with 'bundle/': {}", name);
        }
    }
}