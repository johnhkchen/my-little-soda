...
async fn test_autonomous_coordinator_with_real_github() {
    ...
    assert!(coordinator_result.is_ok(), "Failed to create coordinator: {:?}", coordinator_result);
    ...
    assert_eq!(status_report.agent_id.as_ref().unwrap(), "real-github-test-agent");
...
async fn test_drift_detection_with_real_github() {
    ...
    let github_client = GitHubClient::new().expect("Failed to create GitHub client");
    let mut detector = StateDriftDetector::new(github_client, "drift-github-test-agent".to_string());
...
async fn test_workflow_with_realistic_github_scenarios() {
    ...
    assert!(assign_result.is_ok(), "Assignment failed: {:?}", assign_result);
    ...
    for i in 1..=3 {
        ...
        assert!(progress_result.is_ok(), "Progress step {} failed: {:?}", i, progress_result);
    }
    ...
    let submit_result = workflow.handle_event(AutonomousEvent::SubmitForReview { 
        pr: realistic_pr 
    }).await;
    assert!(submit_result.is_ok(), "Submit for review failed: {:?}", submit_result);
    
    // Verify state history contains all transitions
    let history = workflow.state_history();
    assert!(history.len() >= 6, "Expected at least 6 state transitions, got {}", history.len());
...
async fn test_github_api_error_handling() {
    ...
    assert!(update_result.is_ok(), "State update should succeed even with non-existent issue");
    ...
    match validation_result {
        Ok(drifts) => {
            ...
            println!("Validation handled non-existent issue gracefully, {} drifts detected", drifts.len());
        }
        Err(e) => {
            ...
            assert!(matches!(e, my_little_soda::autonomous::StateDriftError::GitHubError(_) | 
...
async fn test_autonomous_operation_lifecycle() {
    ...
    match start_result {
        ...
        Err(e) => {
            println!("Autonomous operation completed with expected error: {:?}", e);
        }
    }
...
async fn test_drift_detection_timing() {
    ...
    assert!(validation_duration < Duration::from_secs(30), 
           "Validation took too long: {:?}", validation_duration);
...
async fn test_status_reporting_with_real_github() {
    ...
    let status_report = coordinator.get_status_report().await;
    assert_eq!(status_report.agent_id.as_ref().unwrap(), "status-reporting-agent");
    ...
    assert_eq!(drift_report.validation_health, my_little_soda::autonomous::ValidationHealth::Healthy);
