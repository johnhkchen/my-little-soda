...
"#;
    ...
    content_expectations.insert(
        ...
        ContentExpectation {
            ...
            must_not_contain: vec!["TODO".to_string(), "PLACEHOLDER".to_string()],
            regex_patterns: vec![
    ...
    let content_report =
        ContentValidator::validate_file_contents(repo_path, &content_expectations).unwrap();
...
"#;
    ...
    println!(
        "Content validation (expected to fail): {}",
...
async fn test_git_configuration_validator() {
    ...
    // Run git validation
    let git_report =
        GitConfigValidator::validate_git_configuration(repo_path, &git_expectations).unwrap();
...
async fn test_standard_init_expectations() {
    ...
    println!(
        "Content expectations for {} files",
        content_expectations.len()
    );
...
"#;
    ...
    assert!(
        summary.git_validation_passed || !git_report.errors.is_empty(),
