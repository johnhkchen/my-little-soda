...
impl GitHubApiMock {
    ...
    pub async fn mock_issue_view(
        ...
        let labels_json: Vec<Value> = labels.iter().map(|label| json!({"name": label})).collect();
    ...
    pub async fn mock_create_pull_request(
        &self,
        expected_title: &str,
        expected_head: &str,
        expected_base: &str,
    ...
    pub async fn mock_add_label_to_issue(&self, issue_number: u64, label: &str) {
        ...
        Mock::given(method("POST"))
            .and(path(format!(
                "/repos/test-owner/test-repo/issues/{}/labels",
                issue_number
            )))
            .and(header("authorization", "token mock-token"))
...
impl GitHubScenarioBuilder {
    ...
    pub async fn build(self) -> GitHubApiMock {
        ...
        self.mock
            .mock_repository_info("test-owner", "test-repo")
            .await;
        ...
        for (number, (title, state, labels)) in self.issues {
            ...
            self.mock
                .mock_issue_view(number, &title, &state, label_refs)
                .await;
        }
...
mod tests {
    ...
    async fn test_github_api_mock_setup() {
        ...
        // Set up a mock issue
        mock.mock_issue_view(123, "Test Issue", "open", vec!["bug", "priority-high"])
            .await;
        ...
        let response = client
            .get(&format!(
                "{}/repos/test-owner/test-repo/issues/123",
                mock.base_url
            ))
    ...
    async fn test_pull_request_creation_mock() {
        ...
        mock.mock_create_pull_request("Test PR", "feature-branch", "main", 456)
            .await;
    ...
    async fn test_label_addition_mock() {
        ...
        let response = client
            .post(&format!(
                "{}/repos/test-owner/test-repo/issues/789/labels",
                mock.base_url
            ))
            .header("authorization", "token mock-token")
            .header("content-type", "application/json")
    ...
    async fn test_scenario_builder() {
        ...
        let response = client
            .get(&format!(
                "{}/repos/test-owner/test-repo/issues/100",
                mock.base_url
            ))
            .header("authorization", "token mock-token")
            .send()
        ...
        let response = client
            .get(&format!(
                "{}/repos/test-owner/test-repo/issues/101",
                mock.base_url
            ))
            .header("authorization", "token mock-token")
            .send()
    ...
    async fn test_error_scenarios() {
        ...
        mock.mock_api_error("/repos/test-owner/test-repo/issues/999", 404, "Not Found")
            .await;
    ...
    async fn test_multiple_concurrent_mocks() {
        ...
        mock1
            .mock_issue_view(100, "Issue on Server 1", "open", vec!["server1"])
            .await;
        mock2
            .mock_issue_view(100, "Issue on Server 2", "closed", vec!["server2"])
            .await;
