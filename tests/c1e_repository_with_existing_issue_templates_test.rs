...
async fn test_init_preserves_existing_issue_templates() {
    let env =
        InitCommandTestEnvironment::from_fixture_name("repository_with_existing_issue_templates")
    ...
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/bug_report.md"),
        "Should have bug report template"
    );
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/feature_request.md"),
        "Should have feature request template"
    );
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/config.yml"),
        "Should have template configuration"
    );
    assert!(
        env.has_file(".github/pull_request_template.md"),
        "Should have PR template"
    );
    assert!(
        env.has_file("CONTRIBUTING.md"),
        "Should have contributing guide"
    );

    // Read template content to verify structure
    let bug_report = env
        .read_file(".github/ISSUE_TEMPLATE/bug_report.md")
        .expect("Failed to read bug report template");
    assert!(
        bug_report.contains("name: Bug report"),
        "Bug report should have proper frontmatter"
    );
    assert!(
        bug_report.contains("labels: 'bug'"),
        "Bug report should have labels"
    );
    assert!(
        bug_report.contains("**Describe the bug**"),
        "Bug report should have proper sections"
    );

    let feature_request = env
        .read_file(".github/ISSUE_TEMPLATE/feature_request.md")
        .expect("Failed to read feature request template");
    assert!(
        feature_request.contains("name: Feature request"),
        "Feature request should have proper frontmatter"
    );
    assert!(
        feature_request.contains("labels: 'enhancement'"),
        "Feature request should have labels"
    );

    // Run init command in dry run mode to validate integration
    let result = env
        .run_and_validate_init(1, false, true)
        .await
    ...
    assert!(
        result.success,
        "Init should succeed on repository with issue templates: {:?}",
        result.error_message
    );
    assert!(
        result.matches_expectation(),
        "Init should match expectations"
    );

    // Validate post-init state (dry run should not create files)
    let post_init = env
        .validate_post_init_state(true)
        .expect("Failed to validate post-init state");
    assert!(
        post_init.all_expectations_met(),
        "Post-init validation should pass: {:?}",
        post_init.validation_failures()
    );

    // Verify all existing issue template files are still present and unchanged
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/bug_report.md"),
        "Bug report template should be preserved"
    );
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/feature_request.md"),
        "Feature request template should be preserved"
    );
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/config.yml"),
        "Template config should be preserved"
    );
    assert!(
        env.has_file(".github/pull_request_template.md"),
        "PR template should be preserved"
    );
    assert!(
        env.has_file("CONTRIBUTING.md"),
        "Contributing guide should be preserved"
    );
}
...
async fn test_init_enhances_existing_template_setup() {
    ...
    assert!(
        !env.has_file("my-little-soda.toml"),
        "Clambake config should not be created in dry run"
    );
    assert!(
        !env.has_file(".my-little-soda"),
        "Clambake directory should not be created in dry run"
    );

    // Verify template config maintains expected functionality
    let template_config = env
        .read_file(".github/ISSUE_TEMPLATE/config.yml")
        .expect("Failed to read template config");
    assert!(
        template_config.contains("blank_issues_enabled: false"),
        "Template config should maintain settings"
    );
    assert!(
        template_config.contains("contact_links:"),
        "Template config should maintain contact links"
    );
}
...
async fn test_template_preservation_comprehensive() {
    ...
    for (scenario_name, agents, force, dry_run) in scenarios {
        let env = InitCommandTestEnvironment::from_fixture_name(
        ...
        let result = env
            .run_and_validate_init(agents, force, true)
            .await
            .expect(&format!(
                "Failed to run init for scenario {}",
                scenario_name
            ));

        assert!(
            result.success,
            "Init should succeed for scenario {}: {:?}",
...
fn test_issue_template_behavior_specification() {
    ...
    assert!(
        behavior.should_succeed_without_force,
        "Template repo should allow init"
    );
    assert!(
        behavior.should_create_config,
        "Should create clambake config"
    );
    assert!(
        behavior.should_create_directories,
        "Should create clambake directories"
    );
    assert!(behavior.should_create_labels, "Should create GitHub labels");
    assert!(
        behavior.validation_warnings.is_empty(),
    ...
    for expected_file in &expected_template_files {
        assert!(
            fixture.files.contains_key(*expected_file),
            "Fixture should contain template file: {}",
            expected_file
        );
    }
    ...
    assert!(
        bug_report.contains("name: Bug report"),
        "Bug report should have proper name"
    );
    assert!(
        bug_report.contains("about: Create a report to help us improve"),
        "Bug report should have description"
    );
    assert!(
        bug_report.contains("labels: 'bug'"),
        "Bug report should have labels"
    ...
    assert!(
        bug_report.contains("**To Reproduce**"),
        "Bug report should have reproduction steps"
    );
    assert!(
        bug_report.contains("**Expected behavior**"),
        "Bug report should have expected behavior section"
    );

    // Verify feature request template structure
    let feature_request = fixture
        .files
        .get(".github/ISSUE_TEMPLATE/feature_request.md")
        .unwrap();
    assert!(
        feature_request.contains("name: Feature request"),
        "Feature request should have proper name"
    );
    assert!(
        feature_request.contains("labels: 'enhancement'"),
        "Feature request should have enhancement label"
    );
    assert!(
        feature_request
            .contains("**Is your feature request related to a problem? Please describe.**"),
        "Feature request should have problem section"
    );

    // Verify template configuration
    let template_config = fixture
        .files
        .get(".github/ISSUE_TEMPLATE/config.yml")
        .unwrap();
    assert!(
        template_config.contains("blank_issues_enabled: false"),
        "Template config should disable blank issues"
    );
    assert!(
        template_config.contains("contact_links:"),
        "Template config should have contact links"
    );
    assert!(
        template_config.contains("Community Discord"),
        "Template config should reference community resources"
    );
}
...
async fn test_no_template_metadata_conflicts() {
    ...
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/bug_report.md"),
        "Bug report template preserved"
    );
    assert!(
        env.has_file(".github/ISSUE_TEMPLATE/feature_request.md"),
        "Feature request template preserved"
    );
    assert!(
        env.has_file(".github/pull_request_template.md"),
        "PR template preserved"
    );

    // Verify no template naming conflicts
    assert!(
        !env.has_file(".my-little-soda/ISSUE_TEMPLATE"),
        "No issue template directory conflict"
    );
    assert!(
        !env.has_file(".my-little-soda/pull_request_template.md"),
        "No PR template naming conflict"
    );
    ...
    assert!(
        bug_report.contains("name: Bug report"),
        "Bug report name preserved"
    );
    assert!(
        bug_report.contains("labels: 'bug'"),
        "Bug report labels preserved"
    );
    assert!(
        bug_report.contains("**Describe the bug**"),
        "Bug report sections preserved"
    );

    // Verify contributing guide doesn't conflict with clambake
    let contributing = env
        .read_file("CONTRIBUTING.md")
        .expect("Failed to read contributing guide");
    assert!(
        contributing.contains("# Contributing Guide"),
        "Contributing guide structure preserved"
    );
    assert!(
        contributing.contains("## Issue Templates"),
        "Contributing guide template documentation preserved"
    );
}
...
async fn test_complete_c1e_scenario() {
    ...
    assert!(
        dry_run_result.matches_expectation(),
        "Dry run should match expectations"
    );
    ...
    for file in &template_files {
        assert!(
            env.has_file(file),
            "Template file {} should be preserved",
            file
        );
    }
    ...
    assert!(
        bug_report.contains("name: Bug report"),
        "Bug report template should maintain structure"
    );
    assert!(
        bug_report.contains("labels: 'bug'"),
        "Bug report template should maintain labels"
    );
