...
async fn test_init_respects_complex_directory_structure() {
    ...
    assert!(
        env.has_file("src/config/mod.rs"),
        "Should have nested src/config module"
    );
    assert!(
        env.has_file("src/config/database.rs"),
        "Should have database config module"
    );
    assert!(
        env.has_file("src/config/logging.rs"),
        "Should have logging config module"
    );
    assert!(
        env.has_file("src/utils/helpers.rs"),
        "Should have utility helpers"
    );
    assert!(
        env.has_file("src/utils/validation.rs"),
        "Should have validation utilities"
    );
    assert!(
        env.has_file("crates/shared/src/lib.rs"),
        "Should have workspace crate structure"
    );
    assert!(
        env.has_file("services/api/src/main.rs"),
        "Should have service structure"
    );
    assert!(
        env.has_file("docs/architecture.md"),
        "Should have documentation directory"
    );
    assert!(
        env.has_file("config/development.toml"),
        "Should have configuration directory"
    );

    // Verify workspace structure
    let workspace_config = env
        .read_file("Cargo.toml")
        .expect("Failed to read workspace Cargo.toml");
    assert!(
        workspace_config.contains("[workspace]"),
        "Should have workspace configuration"
    );
    assert!(
        workspace_config.contains("members = [\"crates/*\", \"services/*\"]"),
    ...
    assert!(
        result.success,
        "Init should succeed on repository with complex directory structure: {:?}",
    ...
    assert!(
        env.has_file("src/config/mod.rs"),
        "Nested config module should be preserved"
    );
    assert!(
        env.has_file("crates/shared/src/types.rs"),
        "Workspace structure should be preserved"
    );
    assert!(
        env.has_file("services/api/src/main.rs"),
        "Service directory should be preserved"
    );
    assert!(
        env.has_file("docs/deployment/README.md"),
        "Nested docs should be preserved"
    );
    assert!(
        env.has_file("config/production.toml"),
        "Config directory should be preserved"
    );
}
...
async fn test_init_file_placement_respects_structure() {
    ...
    assert!(
        !env.has_file("src/.my-little-soda"),
        "Clambake should not create files in src/"
    );
    assert!(
        !env.has_file("crates/.my-little-soda"),
        "Clambake should not create files in crates/"
    );
    assert!(
        !env.has_file("services/.my-little-soda"),
        "Clambake should not create files in services/"
    );
    assert!(
        !env.has_file("docs/.my-little-soda"),
        "Clambake should not create files in docs/"
    );
    assert!(
        !env.has_file("config/.my-little-soda"),
        "Clambake should not create files in config/"
    );
    ...
    for dir in &original_directories {
        ...
        assert!(
            dir_still_has_content,
            "Directory {} should still contain expected files after init",
            dir
        );
    }
}
...
fn test_directory_handling_behavior_specification() {
    ...
    assert!(
        ...
        "Complex directory repo should allow init"
    );
    assert!(
    ...
    for expected_file in &expected_directories {
        assert!(
            ...
            "Fixture should contain nested file: {}",
            expected_file
        );
    }
    ...
    assert!(
        workspace_toml.contains("members = [\"crates/*\", \"services/*\"]"),
        "Should define workspace members"
    );

    // Verify module structure
    let config_mod = fixture.files.get("src/config/mod.rs").unwrap();
    assert!(
        config_mod.contains("pub mod database;"),
        "Should have nested module declarations"
    );
    assert!(
        config_mod.contains("pub mod logging;"),
        "Should have nested module declarations"
    );
    ...
    assert!(
        api_service_toml.contains("shared = { path = \"../../crates/shared\" }"),
...
async fn test_no_directory_structure_conflicts() {
    ...
    assert!(
        result.success,
        "Init should succeed without directory conflicts"
    );
    ...
    assert!(
        !env.has_file(".my-little-soda/src"),
        "No src directory conflict"
    );
    assert!(
        !env.has_file(".my-little-soda/crates"),
        "No crates directory conflict"
    );
    assert!(
        !env.has_file(".my-little-soda/services"),
        "No services directory conflict"
    );
    assert!(
        !env.has_file(".my-little-soda/docs"),
        "No docs directory conflict"
    );
    assert!(
        !env.has_file(".my-little-soda/config"),
        "No config directory conflict"
    );
    ...
    assert!(
        workspace_config.contains("complex-project"),
        "Project name preserved"
    );

    // Verify service dependencies are preserved
    let api_service_config = env
        .read_file("services/api/Cargo.toml")
        .expect("Failed to read API service config");
    assert!(
        api_service_config.contains("shared = { path = \"../../crates/shared\" }"),
...
async fn test_complete_c1d_scenario() {
    ...
    for file in &structure_files {
        assert!(
            env.has_file(file),
            "Directory structure file {} should exist",
            file
        );
    }
    ...
    for file in &structure_files {
        assert!(
            env.has_file(file),
            "Directory structure file {} should be preserved",
            file
        );
    }
    ...
    assert!(
        !env.has_file("src/my-little-soda.toml"),
        "Config should not be in src/"
    );
    assert!(
        !env.has_file("crates/my-little-soda.toml"),
        "Config should not be in crates/"
    );
    assert!(
        !env.has_file("services/my-little-soda.toml"),
        "Config should not be in services/"
    );
    ...
    println!(
        "📄 Summary: Init respects complex directory structures and places files appropriately"
    );
}
