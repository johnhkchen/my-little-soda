...
fn test_doctor_verbose_comprehensive_output() {
    ...
    cmd.arg("doctor")
        ...
        .stdout(predicate::str::contains("🏥 COMPREHENSIVE HEALTH RECOMMENDATIONS"))
...
fn test_doctor_diagnostic_status_handling() {
    ...
    if let Ok(json_value) = serde_json::from_str::<Value>(&stdout) {
        ...
        assert!(summary.get("total_checks").is_some(), "total_checks missing from summary");
        assert!(summary.get("passed").is_some(), "passed count missing from summary");
        assert!(summary.get("failed").is_some(), "failed count missing from summary");
        assert!(summary.get("warnings").is_some(), "warnings count missing from summary");
        
        // Verify that individual checks have proper status and message structure
        let checks = json_value["checks"].as_object().unwrap();
        for (_check_name, check_data) in checks {
            assert!(check_data.get("status").is_some(), "Check missing status field");
            assert!(check_data.get("message").is_some(), "Check missing message field");
    ...
    if let Ok(json_value) = serde_json::from_str::<Value>(&stdout) {
        ...
        for check_name in &workflow_checks {
            if let Some(check_data) = checks.get(*check_name) {
                assert!(check_data.get("status").is_some(), 
                       "{} missing status", check_name);
                assert!(check_data.get("message").is_some(), 
                       "{} missing message", check_name);
            }
        }
    }
}
...
fn test_doctor_complete_workflow() {
    ...
    assert!(serde_json::from_str::<Value>(&json_stdout).is_ok(), 
...
fn test_doctor_performance() {
    ...
    assert!(duration.as_secs() < 30, 
           "Doctor command took too long: {:?}", duration);
}