...
impl ApiRateLimitValidator {
    ...
    pub async fn validate_api_efficiency(&mut self) -> Result<ApiEfficiencyComparison> {
        ...
        println!(
            "  • Simulated rate limit: {} calls/hour",
            self.config.simulated_rate_limit
        );
        println!(
            "  • Target API reduction: {:.1}%",
        ...
        let comparison =
            self.calculate_efficiency_comparison(&individual_metrics, &bundled_metrics);
    ...
    async fn measure_individual_pr_api_usage(&mut self) -> Result<ApiUsageMetrics> {
        println!(
            "  📋 Simulating individual PR creation for {} issues",
            self.config.total_issues
        );
        ...
        for issue_idx in 1..=self.config.total_issues {
            ...
            let calls_for_issue = self
                .simulate_individual_pr_api_calls(issue_idx as u64)
                .await;
        ...
        let estimated_hourly_usage =
            (total_calls as f64 / duration.as_secs_f64() * 3600.0) as usize;
        let rate_limit_utilization =
            estimated_hourly_usage as f64 / self.config.simulated_rate_limit as f64;
    ...
    async fn measure_bundled_api_usage(&mut self) -> Result<ApiUsageMetrics> {
        println!(
            "  🚄 Simulating bundled PR creation for {} issues",
            self.config.total_issues
        );
        ...
        for bundle_idx in 1..=num_bundles {
            ...
            if issues_in_bundle > 1 {
                println!(
                    "    Creating bundle {} with {} issues...",
                    bundle_idx, issues_in_bundle
                );
                let calls_for_bundle = self
                    .simulate_bundle_api_calls(bundle_idx as u64, issues_in_bundle)
                    .await;
                ...
                let calls_for_issue = self
                    .simulate_individual_pr_api_calls(((bundle_idx - 1) * bundle_size + 1) as u64)
                    .await;
                total_calls += calls_for_issue;
            }
        }
    ...
    fn calculate_efficiency_comparison(
        ...
        let api_reduction_percentage = ((individual.total_api_calls - bundled.total_api_calls)
            as f64
            / individual.total_api_calls as f64)
            * 100.0;

        let efficiency_improvement_factor =
            individual.api_calls_per_issue / bundled.api_calls_per_issue;

        let rate_limit_improvement = ((individual.rate_limit_utilization
            - bundled.rate_limit_utilization)
            / individual.rate_limit_utilization)
            * 100.0;
    ...
    fn print_api_metrics(&self, approach: &str, metrics: &ApiUsageMetrics) {
        ...
        println!(
            ...
            metrics.api_calls_per_issue
        );
        println!(
            "    • Rate limit utilization: {:.1}%",
            metrics.rate_limit_utilization * 100.0
        );
        println!(
            "    • Estimated hourly usage: {}",
            metrics.estimated_hourly_usage
        );
        println!(
            "    • Sustainability score: {:.1}/1.0",
    ...
    fn print_efficiency_comparison(&self, comparison: &ApiEfficiencyComparison) {
        ...
        println!(
            "    • API reduction: {:.1}%",
            comparison.api_reduction_percentage
        );
        println!(
            "    • Efficiency improvement: {:.1}x",
            comparison.efficiency_improvement_factor
        );
        println!(
            "    • Rate limit improvement: {:.1}%",
            comparison.rate_limit_improvement
        );
        println!(
            "    • Target ({:.1}% reduction): {}",
            self.config.target_api_reduction,
...
mod api_rate_limit_tests {
    ...
    async fn test_api_rate_limit_validation_standard_load() {
        ...
        let comparison = validator
            .validate_api_efficiency()
            .await
            .expect("API validation should complete successfully");

        // Validate core API efficiency requirements
        assert!(
            comparison.api_reduction_percentage > 0.0,
            "Should show API call reduction"
        );
        assert!(
            comparison.efficiency_improvement_factor > 1.0,
            "Should show efficiency improvement"
        );
        assert!(
            comparison.bundled_approach_metrics.api_calls_per_issue
    ...
    async fn test_high_volume_api_efficiency() {
        ...
        assert!(
            comparison.bundled_approach_metrics.total_api_calls
                < comparison.individual_pr_metrics.total_api_calls,
            "Bundled approach should use fewer total API calls"
        );

        assert!(
            comparison.bundled_approach_metrics.rate_limit_utilization
        ...
        assert!(
            comparison.bundled_approach_metrics.rate_limit_utilization < 0.9,
            "Should not exceed 90% rate limit utilization"
        );

        println!("✅ High volume API efficiency test completed");
        println!(
            "Rate limit utilization: {:.1}% (bundled) vs {:.1}% (individual)",
    ...
    async fn test_api_call_breakdown_analysis() {
        ...
        if let (Some(individual_labels), Some(bundled_labels)) = (
            individual_breakdown.get("add_labels"),
            bundled_breakdown.get("batch_add_labels"),
        ) {
            println!(
                "Label operations: {} (individual) vs {} (bundled batched)",
    ...
    async fn test_rate_limit_sustainability() {
        ...
        assert!(
            comparison.bundled_approach_metrics.sustainability_score >= 0.7,
            "Bundled approach should have good sustainability score"
        );

        assert!(
            comparison.bundled_approach_metrics.rate_limit_utilization <= 0.8,
        ...
        println!(
            "Sustainability scores: {:.2} (bundled) vs {:.2} (individual)",
    ...
    async fn test_api_efficiency_target_achievement() {
        ...
        assert!(
            comparison.api_reduction_percentage >= 40.0,
            "Should achieve at least 40% API reduction"
        );
        assert!(
            comparison.efficiency_improvement_factor >= 2.0,
        ...
        if !comparison.target_achieved {
            assert!(
                comparison.api_reduction_percentage >= target_api_reduction * 0.8,
                "Should achieve at least 80% of the target reduction"
            );
            println!(
                "⚠️ Target not fully achieved but performance is acceptable ({:.1}% of target)",
