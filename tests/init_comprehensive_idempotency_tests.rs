...
use fixtures::test_harness::{TestHarness, helpers::simple_harness};
...
async fn test_state_preservation_between_calls() {
    ...
    let custom_file = harness.create_file("custom_file.txt", "important data").unwrap();
    ...
    for i in 1..=5 {
        ...
        assert!(harness.path().join("custom_dir/nested.txt").exists(),
...
async fn test_rapid_successive_calls() {
    ...
    for i in 0..5 {
        ...
        assert!(result.is_ok(), "Rapid call {} failed: {:?}", i, result.err());
    }
