...
pub enum FailureScenario {
    RemoveLabelFailure { issue_number: u64, error_msg: String },
    AddLabelFailure { issue_number: u64, error_msg: String },
    ApiFailure { error_msg: String },
}
...
impl MockGitHubClient {
    ...
    fn simulate_gh_command(&self, args: &[&str]) -> Result<Output, IoError> {
        ...
        let repo_targeted = args.contains(&"-R") && args.len() > args.iter().position(|&x| x == "-R").unwrap() + 1;
        
        // Check for simulated failures
        let failures = self.simulate_failures.lock().unwrap();
        for failure in failures.iter() {
            match failure {
                FailureScenario::RemoveLabelFailure { issue_number, error_msg } => {
                    ...
                    if args.contains(&"--remove-label") && args.contains(&issue_str.as_str()) {
                        ...
                        let failed_status = std::process::Command::new("false").status()
                ...
                FailureScenario::AddLabelFailure { issue_number, error_msg } => {
                    let issue_str = issue_number.to_string();
        ...
        // Successful response
        let success_status = std::process::Command::new("echo").status()
...
impl GitHubOps for MockGitHubClient {
    async fn remove_label_from_issue(&self, issue_number: u64, label_name: &str) -> Result<(), GitHubError> {
        ...
        let issue_str = issue_number.to_string();
        let args = vec!["issue", "edit", &issue_str, "-R", &repo, "--remove-label", label_name];
        let args_vec: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    Err(GitHubError::IoError(IoError::new(
                        ErrorKind::Other,
    ...
    async fn add_label_to_issue(&self, issue_number: u64, label_name: &str) -> Result<(), GitHubError> {
        let repo = format!("{}/{}", self.owner(), self.repo());
        let issue_str = issue_number.to_string();
        let args = vec!["issue", "edit", &issue_str, "-R", &repo, "--add-label", label_name];
        let args_vec: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        let repo_targeted = args.contains(&"-R");
    ...
    async fn fetch_issues(&self) -> Result<Vec<octocrab::models::issues::Issue>, GitHubError> {
        let repo = format!("{}/{}", self.owner(), self.repo());
        let args = vec!["issue", "list", "-R", &repo, "--json", "number,title,labels,assignees"];
        let args_vec: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        let repo_targeted = args.contains(&"-R");
...
mod tests {
    ...
    async fn test_label_removal_includes_repo_targeting() {
        ...
        match &calls[0] {
            GhCliCall::RemoveLabel { issue_number, label, repo_targeted, command_args } => {
    ...
    async fn test_issue_fetching_includes_repo_targeting() {
        ...
        match &calls[0] {
            GhCliCall::FetchIssues { repo_targeted, command_args } => {
                assert!(repo_targeted, "gh CLI call must include -R repo targeting");
                assert!(command_args.contains(&"-R".to_string()));
                assert!(command_args.contains(&"johnhkchen/clambake".to_string()));
            }
    ...
    async fn test_label_removal_failure_detection() {
        ...
        let client = MockGitHubClient::new("johnhkchen", "clambake")
            .with_failure_scenario(FailureScenario::RemoveLabelFailure {
                issue_number: 95,
        ...
        assert!(error_msg.contains("Label 'agent001' not found"), 
    ...
    async fn test_label_addition_failure_detection() {
        ...
        let client = MockGitHubClient::new("johnhkchen", "clambake")
            .with_failure_scenario(FailureScenario::AddLabelFailure {
                issue_number: 95,
        ...
        let error = result.unwrap_err();
        let error_msg = format!("{:?}", error);
        assert!(error_msg.contains("does not exist in repository"), 
                "Error should contain failure message: {}", error_msg);
    }
    ...
    async fn test_completed_work_never_reassigned_detection() {
        ...
        assert!(!assignable_numbers.contains(&93), 
                "Completed issue #93 should not be assignable");
        assert!(assignable_numbers.contains(&95), 
    ...
    async fn test_agent_lifecycle_state_transitions() {
        ...
        let calls = client.get_gh_calls();
        assert_eq!(calls.len(), 3, "Should have 3 gh CLI calls for complete lifecycle");
        ...
        for call in &calls {
            match call {
                ...
                GhCliCall::RemoveLabel { repo_targeted, .. } => {
                    assert!(repo_targeted, "All lifecycle operations must use repo targeting");
                }
                _ => {}
            }
        }
