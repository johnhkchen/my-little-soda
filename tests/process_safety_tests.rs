...
impl ProcessSafetyTestHelper {
    ...
    pub fn simulate_bundler_startup(&self) -> Result<()> {
        if self.can_acquire_lock() {
            ...
            Err(anyhow!("Another bundler is already running. Only one bundler can run at a time."))
        }
    }
...
mod tests {
    ...
    fn test_single_bundler_lock_acquisition() {
        ...
        assert!(helper.can_acquire_lock(), "Should be able to acquire lock initially");
        
        // Bundler startup should work
        let bundler_result = helper.simulate_bundler_startup();
        assert!(bundler_result.is_ok(), "First bundler should acquire lock successfully");
    }
    ...
    fn test_lock_file_behavior() {
        ...
        assert!(!helper.lock_file_exists() || helper.can_acquire_lock(), 
        ...
        assert!(helper.can_acquire_lock(), "Should be able to acquire lock after first attempt");
    }
    ...
    fn test_fd_lock_basic_functionality() {
        ...
        assert!(helper.can_acquire_lock(), "Should be able to acquire lock after previous lock dropped");
    }
    ...
    fn test_concurrent_lock_behavior() {
        ...
        assert!(successful_count > 0, "At least some lock attempts should succeed");
    }
    ...
    fn test_error_conditions() {
        ...
        assert!(!bad_helper.can_acquire_lock(), "Should fail with bad lock file path");
        ...
        assert!(helper.simulate_bundler_startup().is_ok(), "Normal case should still work");
    }
    ...
    fn test_bundler_integration() {
        ...
        assert!(helper.can_acquire_lock(), "Should still be able to acquire lock");
    }
}