    ...
    fn validate_file_existence(
        ...
        for file_path in expected_files {
            ...
            if full_path.exists() && full_path.is_file() {
                ...
                report
                    .errors
                    .push(format!("Expected file not found: {}", file_path));
            }
        }
        ...
        for dir_path in expected_directories {
            ...
            if full_path.exists() && full_path.is_dir() {
                ...
                report
                    .errors
                    .push(format!("Expected directory not found: {}", dir_path));
            }
        }
...
async fn test_automated_validation_file_existence() {
    ...
    assert_eq!(
        fs_report.directories_found.len(),
        3,
        "Should find 3 directories"
    );
    assert_eq!(
        fs_report.files_missing.len(),
        0,
...
"#;
    ...
    // Run validation
    let content_report =
        SimpleContentValidator::validate_file_contents(repo_path, &content_expectations).unwrap();
    ...
    assert_eq!(
        content_report.passed_files.len(),
        1,
        "Should have 1 passing file"
    );
    assert_eq!(
        content_report.failed_files.len(),
        0,
        "Should have no failing files"
    );
...
"#;
    ...
    println!(
        "Content validation success (should be false): {}",
    ...
    assert!(
        !content_report.success,
        "Validation should fail for bad configuration"
    );
    assert!(
        content_report.errors.len() >= 4,
        "Should detect multiple specific errors"
    );
    ...
    assert!(
        ...
        "Should detect missing required patterns"
    );
    assert!(
    ...
    assert!(
        ...
        "Should detect line count issues"
    );
...
fn test_validation_expectations_structure() {
    ...
    let content_expectation = ContentExpectation {
        must_contain: vec!["[github]".to_string(), "[agents]".to_string()],
    ...
    println!(
        "  Must not contain: {:?}",
        content_expectation.must_not_contain
    );
