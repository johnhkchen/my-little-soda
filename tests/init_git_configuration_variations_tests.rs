...
async fn test_init_with_multiple_existing_branches() {
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with multiple branches: {:?}",
        result.err()
    );
...
async fn test_init_with_detached_head() {
    ...
    let first_commit_hash = log_output
        .lines()
        .next()
        .unwrap()
        .split_whitespace()
...
async fn test_init_with_git_stash_present() {
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with stash: {:?}",
        result.err()
    );
...
"#,
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with submodule config: {:?}",
        result.err()
    );
...
"#;
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = std::fs::metadata(".git/hooks/pre-commit")
            .unwrap()
            .permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(".git/hooks/pre-commit", perms).unwrap();

        let mut perms = std::fs::metadata(".git/hooks/post-commit")
            .unwrap()
            .permissions();
        perms.set_mode(0o755);
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with git hooks: {:?}",
        result.err()
    );
...
async fn test_init_with_large_git_history() {
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with large history: {:?}",
        result.err()
    );
...
async fn test_init_with_custom_git_config() {
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with custom git config: {:?}",
        result.err()
    );
...
async fn test_init_with_git_lfs_configuration() {
    ...
    std::fs::write(
        ".gitattributes",
        r#"*.zip filter=lfs diff=lfs merge=lfs -text
...
"#,
    ...
    assert!(
        result.is_ok(),
        "Init should handle repos with LFS config: {:?}",
        result.err()
    );
