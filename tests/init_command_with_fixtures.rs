...
use fixtures::init_integration::{
    assertions, InitCommandBatchTester, InitCommandTestEnvironment, TestScenario,
};
use fixtures::repository_states::{RepositoryFixtureLoader, RepositoryStateFixture};
use fixtures::validation_helpers::{InitCommandValidator, RepositoryStateValidator};
...
async fn test_init_on_empty_repository_with_fixture() {
    ...
    assert!(
        env.has_file("README.md"),
        "Empty repository should have README"
    );
    assert!(
...
async fn test_init_with_existing_config_using_fixtures() {
    let env =
        InitCommandTestEnvironment::from_fixture_name("repository_with_partial_initialization")
            .expect("Failed to create test environment");

    // Verify existing configuration
    assert!(
        env.has_file("my-little-soda.toml"),
        "Should have existing config"
    );
    let existing_config = env
        .read_file("my-little-soda.toml")
        .expect("Failed to read existing config");
    assert!(
        existing_config.contains("old-owner"),
        "Should contain old owner"
    );

    // Test without force - should fail
    let result_no_force = env
        .run_and_validate_init(1, false, true)
        .await
        .expect("Failed to run init without force");
    assertions::assert_init_fails_without_force(
        &result_no_force,
        "repository_with_partial_initialization",
    );

    // Test with force - should succeed
    let result_with_force = env
        .run_and_validate_init(1, true, true)
        .await
        .expect("Failed to run init with force");
    assertions::assert_init_succeeds_with_force(
        &result_with_force,
        "repository_with_partial_initialization",
    );
}
...
async fn test_init_with_repository_conflicts_using_fixtures() {
    ...
    assert!(
        main_rs.contains("<<<<<<< HEAD"),
        "Should have conflict markers"
    );
    assert!(
        main_rs.contains(">>>>>>> feature-branch"),
        "Should have conflict markers"
    );
...
async fn test_init_dry_run_comprehensive_fixture_coverage() {
    let results = InitCommandBatchTester::test_all_fixtures(1, false, true)
        .await
    ...
    for result in &results.results {
        assert!(
            result.passed(),
            "Fixture {} should pass dry run test: {}",
            result.fixture_name,
...
async fn test_force_flag_across_all_fixtures() {
    ...
    let results = InitCommandBatchTester::test_scenario_across_fixtures(force_scenario)
        .await
    ...
    assert!(
        results.all_passed(),
        "All fixtures should pass with force flag"
    );
    assert_eq!(
        results.failed_count(),
        0,
        "No fixtures should fail with force flag"
    );

    // Validate cross-fixture consistency
    let cross_validation_results: Vec<_> = results
        .results
        .iter()
        .map(|r| {
            (
                r.fixture_name.clone(),
                r.command_result.clone(),
                r.post_init_validation.clone(),
            )
        })
        .collect();
    ...
    assert!(
        cross_report.passed(),
        "Cross-fixture validation should pass for force scenario"
    );
    assert_eq!(
        cross_report.success_rate(),
        1.0,
...
async fn test_multi_agent_setup_with_realistic_repositories() {
    ...
    for &agent_count in &agent_counts {
        let result = env
            .run_and_validate_init(agent_count, false, true)
            .await
        ...
        assert!(
            result.success,
            "Init should succeed with {} agents on existing project",
            agent_count
        );
    }
}
...
async fn test_error_conditions_with_fixture_validation() {
    ...
    for fixture_name in &problematic_fixtures {
        ...
        assert!(
            comprehensive_report.passed(),
            "Comprehensive validation should pass for fixture '{}'",
            fixture_name
        );
    }
}
...
async fn test_edge_cases_now_accessible_with_fixtures() {
    ...
    let env_with_code =
        InitCommandTestEnvironment::from_fixture_name("repository_with_existing_files")
            .expect("Failed to create environment");

    // Verify it has realistic project structure
    assert!(
        env_with_code.has_file("Cargo.toml"),
        "Should have Cargo.toml"
    );
    assert!(env_with_code.has_file("src/main.rs"), "Should have main.rs");
    assert!(
        env_with_code.has_file("src/lib.rs"),
        "Should have lib.rs with tests"
    );

    let result = env_with_code
        .run_and_validate_init(1, false, true)
        .await
    ...
    let env_with_conflicts =
        InitCommandTestEnvironment::from_fixture_name("repository_with_conflicts")
            .expect("Failed to create conflict environment");

    let main_content = env_with_conflicts
        .read_file("src/main.rs")
        .expect("Failed to read conflicted file");
    assert!(
        main_content.contains("Version from main branch"),
        "Should have main branch content"
    );
    assert!(
        main_content.contains("Version from feature branch"),
        "Should have feature branch content"
    );

    let result = env_with_conflicts
        .run_and_validate_init(1, false, true)
        .await
    ...
    assert!(
        !result.success,
        "Init should fail on repository with conflicts"
    );
    assertions::assert_result_matches_expectation(&result, "repository_with_conflicts");
}
...
async fn test_comprehensive_init_scenarios() {
    ...
    for scenario in scenarios {
        ...
        let results = InitCommandBatchTester::test_scenario_across_fixtures(scenario.clone())
            .await
            .expect(&format!("Scenario '{}' failed", scenario.description));

        println!(
            "  Results: {}/{} fixtures passed",
...
async fn test_fixture_system_performance_characteristics() {
    ...
    println!(
        "Fixture loading time: {:?} for {} fixtures",
        loading_time,
        fixtures.len()
    );
    ...
    let start = Instant::now();
    let _results = InitCommandBatchTester::test_all_fixtures(1, false, true)
        .await
        .expect("Batch testing failed");
    let batch_testing_time = start.elapsed();
    println!(
        "Batch testing time: {:?} for all fixtures",
        batch_testing_time
    );

    // Performance should be reasonable (these are loose bounds for demonstration)
    assert!(
        loading_time.as_millis() < 1000,
        "Fixture loading should be fast"
    );
    assert!(
        creation_time.as_millis() < 5000,
        "Temp repo creation should be reasonable"
    );
    assert!(
        batch_testing_time.as_secs() < 30,
        "Batch testing should complete in reasonable time"
    );
}
...
async fn test_fixture_extensibility() {
    ...
    assert!(
        env.has_file("custom_config.toml"),
        "Should have custom config file"
    );
    ...
    assert!(
        result.success,
        "Custom fixture should allow successful init"
    );
}
...
async fn test_complete_fixture_integration_workflow() {
    ...
    for fixture in &fixtures {
        let instantiation = RepositoryStateValidator::validate_fixture_instantiation(fixture)
            .await
            .expect("Instantiation validation failed");
        assert!(
            instantiation.successful(),
            "Fixture '{}' should instantiate successfully",
            fixture.name
        );
    }
    ...
    let batch_results = InitCommandBatchTester::test_all_fixtures(1, false, true)
        .await
    ...
    for scenario in scenarios {
        ...
        println!(
            "✅ Scenario '{}' passed for all fixtures",
            scenario.description
        );
    }
