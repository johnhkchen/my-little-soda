...
impl MockArtifactHandler {
    ...
    pub fn upload_bundle_metadata(&self, metadata: serde_json::Value) -> Result<String, String> {
        ...
        self.artifacts.lock().unwrap().insert(artifact_id.clone(), metadata);
        self.upload_calls.lock().unwrap().push(artifact_id.clone());
        
        Ok(artifact_id)
    }
    
    pub fn download_bundle_metadata(&self, artifact_id: &str) -> Result<Option<serde_json::Value>, String> {
        self.download_calls.lock().unwrap().push(artifact_id.to_string());
...
impl MockCIWorkflowCoordinator {
    ...
    pub fn execute_bundling_workflow(&self, ci_mode: bool, force: bool) -> Result<WorkflowExecution, String> {
        ...
        self.workflow_executions.lock().unwrap().push(execution.clone());
        self.performance_metrics.lock().unwrap().insert(execution_id.clone(), duration);
    ...
    fn apply_ci_optimizations(&self) -> Result<(), String> {
        ...
        self.performance_metrics.lock().unwrap().insert("ci_optimizations".to_string(), optimization_time);
...
mod tests {
    ...
    fn test_ci_mode_environment_detection() {
        ...
        assert!(github_actions_env.is_ci_mode(), "GitHub Actions environment should be detected as CI mode");
        assert!(!local_env.is_ci_mode(), "Local environment should not be detected as CI mode");
        
        // And: Artifact handling strategies should differ
        assert_eq!(github_actions_env.get_artifact_handling_strategy(), "optimized");
    ...
    fn test_ci_mode_workflow_execution() {
        ...
        assert!(ci_execution.ci_mode, "CI execution should have CI mode enabled");
        assert!(!standard_execution.ci_mode, "Standard execution should not have CI mode");
        ...
        assert!(ci_duration <= standard_duration, "CI mode should be faster or equal due to optimizations");
    ...
    fn test_ci_mode_artifact_handling() {
        ...
        assert!(coordinator.artifact_handler.get_upload_call_count() > 0, "Artifacts should be uploaded in CI mode");
        ...
        let metadata = coordinator.artifact_handler.download_bundle_metadata(artifact_id).unwrap();
        
        // Then: Metadata should contain execution information
        assert!(metadata.is_some(), "Artifact metadata should be retrievable");
    ...
    fn test_ci_mode_performance_optimization() {
        ...
        let avg_ci_duration: Duration = ci_durations.iter().sum::<Duration>() / ci_durations.len() as u32;
        let avg_std_duration: Duration = standard_durations.iter().sum::<Duration>() / standard_durations.len() as u32;
        
        assert!(avg_ci_duration <= avg_std_duration + Duration::from_millis(10), 
        ...
        assert!(metrics.contains_key("ci_optimizations"), "CI optimization metrics should be tracked");
    }
    ...
    fn test_ci_mode_failure_handling() {
        ...
        assert!(!ci_failure.success, "CI failure should be marked as unsuccessful");
        assert!(!std_failure.success, "Standard failure should be marked as unsuccessful");
        
        // And: CI mode should still upload failure artifacts
        assert!(coordinator.artifact_handler.get_upload_call_count() > 0, 
                "CI mode should upload failure artifacts for debugging");
        
        // And: Error messages should be preserved
        assert!(ci_failure.error_message.is_some(), "CI failure should have error message");
        assert!(std_failure.error_message.is_some(), "Standard failure should have error message");
    }
    ...
    fn test_multiple_ci_workflows_coordination() {
        ...
        for i in 0..workflow_count {
            let execution = coordinator.execute_bundling_workflow(true, i % 2 == 0).unwrap();
            executions.push(execution);
        }
        
        // Then: All workflows should complete successfully
        assert_eq!(executions.len(), workflow_count, "All workflows should execute");
        assert!(executions.iter().all(|e| e.success), "All workflows should succeed");
        
        // And: Each should have unique execution IDs
        let execution_ids: std::collections::HashSet<_> = executions.iter().map(|e| &e.execution_id).collect();
        assert_eq!(execution_ids.len(), workflow_count, "All execution IDs should be unique");
        
        // And: CI optimizations should be applied consistently
        assert!(coordinator.verify_ci_optimizations_applied(), "CI optimizations should be applied to all workflows");
    }
    ...
    fn test_ci_mode_timeout_adjustments() {
        ...
        assert!(timeout_adjustment > Duration::from_secs(0), "CI mode should have timeout adjustments");
    ...
    fn test_ci_mode_artifact_persistence() {
        ...
        let artifacts = coordinator.artifact_handler.list_artifacts();
        assert!(artifacts.len() >= 2, "Multiple workflow artifacts should be persisted");
        ...
        for artifact_id in &artifacts {
            ...
            let metadata = metadata.unwrap();
            assert!(metadata.get("execution_id").is_some(), "Metadata should contain execution ID");
            assert!(metadata.get("workflow_run_id").is_some(), "Metadata should contain workflow run ID");
        }
    }
    ...
    fn test_hybrid_ci_and_local_workflow_compatibility() {
        ...
        let ci_execution = ci_coordinator.execute_bundling_workflow(true, false).unwrap();
        let local_execution = local_coordinator.execute_bundling_workflow(false, false).unwrap();
        ...
        assert!(ci_coordinator.artifact_handler.get_upload_call_count() > 0, "CI should upload artifacts");
        assert_eq!(local_coordinator.artifact_handler.get_upload_call_count(), 0, "Local should not upload artifacts");
    }
}