...
use fixtures::test_metrics_collector::{
    BatchMetricsCollector, SimpleTestTimer,
};
...
async fn test_test_summary_dashboard_basic() -> Result<()> {
    ...
    let test_results = vec![
        create_mock_test_result("ui_tests", "frontend", TestStatus::Passed, Duration::from_millis(150)),
        create_mock_test_result("api_tests", "backend", TestStatus::Passed, Duration::from_millis(300)),
        create_mock_test_result("db_tests", "database", TestStatus::Failed, Duration::from_millis(450)),
        create_mock_test_result("security_tests", "security", TestStatus::Passed, Duration::from_millis(200)),
    ];
...
fn test_test_result_serialization() {
    ...
    let mut test_result = TestResult::new("serialization_test".to_string(), "serialization".to_string());
    ...
    let json_str = serde_json::to_string_pretty(&test_result).expect("Failed to serialize test result");
    assert!(json_str.contains("serialization_test"));
    assert!(json_str.contains("Passed"));
    assert!(json_str.contains("42.5"));
    
    // Test JSON deserialization
    let deserialized: TestResult = serde_json::from_str(&json_str).expect("Failed to deserialize test result");
