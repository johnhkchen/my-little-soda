...
async fn test_clambake_peek_shows_completed_work_bug_reproduction() {
    ...
    let has_completed_work = routable_issues.iter().any(|issue| {
        issue.number == 95 && issue.labels.iter().any(|l| l.name.starts_with("agent"))
    });
...
async fn test_correct_behavior_after_agent_completes_work() {
    ...
    assert_eq!(
        routable_issues.len(),
        1,
        "Should only have 1 routable issue"
    );
    assert_eq!(
        routable_issues[0].number, 96,
        "Should be the ready issue, not completed"
    );

    let has_completed_work = routable_issues.iter().any(|issue| issue.number == 95);
    assert!(
        !has_completed_work,
        "Completed work (no routing labels) should not be routable"
    );
}
...
async fn test_route_ready_to_merge_issues_should_not_appear_in_new_work_queue() {
    ...
    let has_merge_ready = new_work_queue.iter().any(|issue| {
        issue
            .labels
            .iter()
    ...
    assert!(
        !has_merge_ready,
        "Merge-ready work should not appear in new work queue"
    );
}

// Mock the corrected routing logic
fn filter_routable_issues_logic(
