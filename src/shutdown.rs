...
async fn cancel_git_operations() -> Result<()> {
    ...
    }).await.map_err(|_| anyhow::anyhow!("Timeout waiting for git operations to cancel"))?;
...
async fn wait_for_agents_to_finish() -> Result<()> {
    ...
    timeout(Duration::from_secs(30), async {
        ...
        tokio::time::sleep(Duration::from_millis(100)).await;
    }).await.map_err(|_| anyhow::anyhow!("Timeout waiting for agents to finish"))?;
...
async fn finish_current_bundles() -> Result<()> {
    ...
    timeout(Duration::from_secs(60), async {
        ...
        tokio::time::sleep(Duration::from_millis(100)).await;
    }).await.map_err(|_| anyhow::anyhow!("Timeout waiting for bundles to finish"))?;
...
async fn close_github_connections() -> Result<()> {
    ...
    timeout(Duration::from_secs(5), async {
        tokio::time::sleep(Duration::from_millis(100)).await;
    }).await.map_err(|_| anyhow::anyhow!("Timeout waiting for connections to close"))?;
