...
impl PullRequestHandler {
    ...
    pub async fn get_pr_status(&self, pr_number: u64) -> Result<PullRequestStatus, GitHubError> {
        ...
        let (approved_reviews, requested_changes) = match reviews_result {
            Ok(reviews) => {
                ...
                let mut latest_reviews: HashMap<octocrab::models::UserId, &octocrab::models::pulls::Review> = HashMap::new();
    ...
    pub async fn safe_merge_pull_request(
        ...
        if conflict_analysis.has_conflicts || !conflict_analysis.is_mergeable {
            ...
            let recovery_pr = self.create_conflict_recovery_pr(pr_number, recovery_data, issue_handler).await?;
    ...
    pub async fn create_conflict_recovery_pr(
        ...
        issue_handler.add_label_to_issue(work_data.issue_number, "merge-conflict").await?;
        issue_handler.add_label_to_issue(work_data.issue_number, "human-review-required").await?;
        issue_handler.add_label_to_issue(work_data.issue_number, "work-preserved").await?;
