...
impl MetricsReporter {
    pub fn format_metrics_report(metrics: &IntegrationMetrics, detailed: bool) -> String {
        ...
        if let Some(avg_time) = metrics.average_time_to_merge_seconds {
            ...
            report.push_str(&format!(
                "   Avg Time to Merge: {hours:.1}h {minutes:.0}m\n"
            ));
        }
        ...
        if !metrics.agent_metrics.is_empty() {
            ...
            agents.sort_by(|a, b| {
                b.successful_integrations
                    .cmp(&a.successful_integrations)
                    .then_with(|| {
            ...
            for agent in agents {
                ...
                if let Some(avg_time) = agent.average_completion_time_seconds {
                    let hours = avg_time / 3600.0;
                    let minutes = (avg_time % 3600.0) / 60.0;
                    report.push_str(&format!(", avg {hours:.1}h {minutes:.0}m"));
                }
    ...
    pub fn export_metrics_for_monitoring(
        ...
        if !routing_metrics.is_empty() {
            ...
            export.insert(
                "routing_avg_duration_ms".to_string(),
                serde_json::Value::Number(
                    serde_json::Number::from_f64(avg_routing_time)
                        .unwrap_or(serde_json::Number::from(0)),
                ),
            );

            let success_rate = routing_metrics
                .iter()
        ...
        if !utilization_metrics.is_empty() {
            let avg_utilization: f64 = utilization_metrics
                .iter()
                .map(|m| m.utilization_percentage)
                .sum::<f64>()
                / utilization_metrics.len() as f64;

            export.insert(
                "agent_avg_utilization_percentage".to_string(),
                serde_json::Value::Number(
                    serde_json::Number::from_f64(avg_utilization)
                        .unwrap_or(serde_json::Number::from(0)),
                ),
            );
        }
        ...
        export.insert(
            "bottlenecks_critical".to_string(),
