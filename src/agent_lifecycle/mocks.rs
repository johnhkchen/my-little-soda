...
impl MockGitOperations {
    pub fn new() -> Self {
        Self {
            ...
            status: RefCell::new("On branch main\nnothing to commit, working tree clean".to_string()),
    ...
    pub fn set_commits_ahead(&self, base: &str, commits: Vec<String>) {
        self.commits_ahead.borrow_mut().insert(base.to_string(), commits);
    }
    
    pub fn set_commits_behind(&self, base: &str, count: u32) {
        self.commits_behind.borrow_mut().insert(base.to_string(), count);
    }
    
    pub fn set_branch_exists(&self, branch: &str, exists: bool) {
        self.branches.borrow_mut().insert(branch.to_string(), exists);
    }
    
    pub fn set_merge_conflicts(&self, base: &str, files: Vec<String>) {
        self.merge_conflicts.borrow_mut().insert(base.to_string(), files);
    }
...
impl GitOperations for MockGitOperations {
    fn get_current_branch(&self) -> Result<String> {
        self.executed_commands.borrow_mut().push(GitCommand::GetCurrentBranch);
        Ok(self.current_branch.borrow().clone())
    }
    
    fn get_commits_ahead(&self, base: &str) -> Result<Vec<String>> {
        self.executed_commands.borrow_mut().push(GitCommand::GetCommitsAhead { 
            base: base.to_string() 
        });
        Ok(self.commits_ahead.borrow().get(base).cloned().unwrap_or_default())
    }
    
    fn get_commits_behind(&self, base: &str) -> Result<u32> {
        self.executed_commands.borrow_mut().push(GitCommand::GetCommitsBehind { 
            base: base.to_string() 
        });
    ...
    fn checkout_branch(&self, branch: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitCommand::CheckoutBranch { 
    ...
    fn create_branch(&self, name: &str, from: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitCommand::CreateBranch { 
            name: name.to_string(),
            from: from.to_string(),
        });
    ...
    fn delete_branch(&self, name: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitCommand::DeleteBranch { 
            name: name.to_string() 
        });
    ...
    fn commit(&self, message: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitCommand::Commit { 
    ...
    fn get_merge_conflicts(&self, base: &str) -> Result<Vec<String>> {
        self.executed_commands.borrow_mut().push(GitCommand::GetMergeConflicts { 
            base: base.to_string() 
        });
        Ok(self.merge_conflicts.borrow().get(base).cloned().unwrap_or_default())
    }
    
    fn is_clean(&self) -> Result<bool> {
        self.executed_commands.borrow_mut().push(GitCommand::IsClean);
        Ok(*self.is_clean.borrow())
    }
    
    fn get_status(&self) -> Result<String> {
        self.executed_commands.borrow_mut().push(GitCommand::GetStatus);
    ...
    fn remote_branch_exists(&self, _remote: &str, branch: &str) -> Result<bool> {
        Ok(self.branches.borrow().get(&format!("origin/{}", branch)).cloned().unwrap_or(false))
    }
}
...
impl GitHubOperations for MockGitHubOperations {
    fn add_label(&self, issue: u64, label: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitHubCommand::AddLabel {
            issue,
    ...
    fn remove_label(&self, issue: u64, label: &str) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitHubCommand::RemoveLabel {
            issue,
            label: label.to_string(),
        });
    ...
    fn get_issue(&self, issue: u64) -> Result<IssueData> {
        self.executed_commands.borrow_mut().push(GitHubCommand::GetIssue { issue });
        
        self.issues.borrow().get(&issue).cloned()
            .ok_or_else(|| anyhow!("Issue #{} not found", issue))
    }
    
    fn get_labels(&self, issue: u64) -> Result<Vec<String>> {
        self.executed_commands.borrow_mut().push(GitHubCommand::GetLabels { issue });
        
        Ok(self.labels.borrow().get(&issue).cloned().unwrap_or_default())
    }
    
    fn create_pr(&self, title: &str, body: &str, head: &str, base: &str) -> Result<String> {
        self.executed_commands.borrow_mut().push(GitHubCommand::CreatePR {
        ...
        let pr_url = format!("https://github.com/{}/{}/pull/{}", self.owner, self.repo, pr_number);
    ...
    fn merge_pr(&self, number: u64) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitHubCommand::MergePR { number });
    ...
    fn close_pr(&self, number: u64) -> Result<()> {
        self.executed_commands.borrow_mut().push(GitHubCommand::ClosePR { number });
    ...
    fn get_pr(&self, number: u64) -> Result<PRData> {
        self.executed_commands.borrow_mut().push(GitHubCommand::GetPR { number });
        
        self.prs.borrow().get(&number).cloned()
            .ok_or_else(|| anyhow!("PR #{} not found", number))
    }
    
    fn issue_has_label(&self, issue: u64, label: &str) -> Result<bool> {
        Ok(self.labels.borrow()
            .get(&issue)
            .map(|labels| labels.contains(&label.to_string()))
            .unwrap_or(false))
    }
            ...
            Command::GitHub(github_cmd) => {
                match github_cmd {
                    GitHubCommand::AddLabel { issue, label } => {
                        let _ = self.github.add_label(*issue, label);
                        Ok(CommandResult {
                            success: true,
                    ...
                    GitHubCommand::RemoveLabel { issue, label } => {
                        let _ = self.github.remove_label(*issue, label);
                        Ok(CommandResult {
                            success: true,
