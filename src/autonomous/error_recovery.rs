...
use super::workflow_state_machine::{AbandonmentReason, AutonomousWorkflowState};
...
pub enum RecoveryStrategy {
    ...
    Fallback { alternative: AlternativeApproach },
    ...
    AbandonAndReset { reason: AbandonmentReason },
}
...
impl std::fmt::Debug for AutonomousErrorRecovery {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AutonomousErrorRecovery")
            ...
            .field(
                "enable_aggressive_recovery",
                &self.enable_aggressive_recovery,
            )
            .finish()
    }
}
...
impl AutonomousErrorRecovery {
    ...
    pub fn determine_recovery_strategy(&self, error_type: &ErrorType) -> RecoveryStrategy {
        match error_type {
            ErrorType::GitOperationFailed { operation, .. } => match operation.as_str() {
            ...
            ErrorType::GitHubAPIError { status, .. } => match *status {
            ...
            ErrorType::StateInconsistency { .. } => RecoveryStrategy::AutomatedFix {
                fix_type: FixType::ConfigurationAdjustment,
                confidence: ConfidenceLevel::High,
            },
        }
    ...
    pub async fn execute_recovery_strategy(
        ...
        let success = match strategy {
            ...
            RecoveryStrategy::AutomatedFix {
                ...
                match self
                    .execute_automated_fix(fix_type, confidence, &mut recovery_actions)
                    .await
                {
                    Ok(_) => true,
                    Err(e) => {
            ...
            RecoveryStrategy::Fallback { ref alternative } => {
                match self
                    .execute_fallback_strategy(*alternative, &mut recovery_actions)
                    .await
                {
                    Ok(_) => true,
                    Err(e) => {
            ...
            RecoveryStrategy::Escalate {
                ref urgency,
                context: ref escalation_context,
            } => {
                self.execute_escalation(
                    *urgency,
                    escalation_context.clone(),
                    &mut recovery_actions,
                )
                .await?;
            ...
            RecoveryStrategy::AbandonAndReset { ref reason } => {
                self.execute_abandonment(reason.clone(), &mut recovery_actions)
                    .await?;
        ...
        while attempt < max_attempts {
            ...
            let retry_success = match error_type {
                ErrorType::GitOperationFailed { operation, .. } => {
                    self.retry_git_operation(operation, recovery_actions)
                        .await?
                }
    ...
    async fn execute_automated_fix(
        ...
        match fix_type {
            FixType::MergeConflictResolution => {
                self.resolve_merge_conflicts_automatically(recovery_actions)
                    .await?;
            }
            FixType::TestFailureFix => {
                self.fix_test_failures_automatically(recovery_actions)
                    .await?;
            }
            FixType::BuildErrorFix => {
                self.fix_build_errors_automatically(recovery_actions)
                    .await?;
            }
            FixType::DependencyUpdate => {
                self.update_dependencies_automatically(recovery_actions)
                    .await?;
            }
            FixType::ConfigurationAdjustment => {
                self.adjust_configuration_automatically(recovery_actions)
                    .await?;
            }
    ...
    async fn resolve_merge_conflicts_automatically(
        ...
        let index = repo
            .index()
            .into_recovery_error("getting repository index")?;
    ...
    async fn fix_test_failures_automatically(
        ...
        let output = tokio::process::Command::new("cargo")
            ...
            .output()
            .await
            .into_recovery_error("running cargo test")?;
        ...
        let non_critical_tests: Vec<String> = failed_tests
            .iter()
            .filter(|test| {
    ...
    async fn fix_build_errors_automatically(
        ...
        Err(RecoveryError::GitError(
            "Build errors could not be resolved automatically".to_string(),
        ))
    }
    ...
    async fn update_dependencies_automatically(
        ...
        Err(RecoveryError::GitError(
            "Dependency issues could not be resolved automatically".to_string(),
        ))
    }
    ...
    async fn resolve_simple_conflicts(
        ...
        for line in content.lines() {
            if line.starts_with("<<<<<<< ") {
                ...
                let resolution =
                    self.resolve_conflict_sections(&our_section, &their_section, file_path);
    ...
    fn get_most_effective_strategies(&self) -> Vec<(String, f64)> {
        ...
        let mut effectiveness: Vec<_> = strategy_stats
            .into_iter()
