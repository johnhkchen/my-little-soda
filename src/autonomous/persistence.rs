...
use super::{
    error_recovery::AutonomousRecoveryAttempt, workflow_state_machine::StateTransitionRecord,
...
pub trait StatePersistence {
    ...
    async fn load_state(
        &self,
        agent_id: &str,
    ) -> Result<Option<PersistentWorkflowState>, PersistenceError>;
    ...
    async fn list_checkpoints(
        &self,
        agent_id: &str,
    ) -> Result<Vec<CheckpointInfo>, PersistenceError>;
...
impl FileSystemPersistence {
    ...
    fn get_state_file_path(&self, agent_id: &str) -> PathBuf {
        self.config
            ...
            .join(format!("{agent_id}.state.json"))
    }
    ...
    fn get_checkpoint_dir(&self, agent_id: &str) -> PathBuf {
        self.config
            .persistence_directory
            .join(format!("{agent_id}_checkpoints"))
    }

    /// Get the checkpoint file path
    fn get_checkpoint_file_path(&self, agent_id: &str, checkpoint_id: &str) -> PathBuf {
        self.get_checkpoint_dir(agent_id)
            .join(format!("{checkpoint_id}.checkpoint.json"))
    }
    ...
    async fn restore_from_checkpoint(
        ...
        if !checkpoint_file.exists() {
            return Err(PersistenceError::IoError(std::io::Error::new(
                ...
                format!("Checkpoint {checkpoint_id} not found for agent {agent_id}"),
            )));
        }
...
impl StatePersistenceManager {
    ...
    pub async fn start_auto_save(
        &mut self,
        mut state_receiver: tokio::sync::mpsc::Receiver<PersistentWorkflowState>,
        ...
        let interval_duration =
            tokio::time::Duration::from_secs(self.config.auto_save_interval_minutes as u64 * 60);
    ...
    pub async fn restore_from_checkpoint(
        ...
        self.persistence
            .restore_from_checkpoint(agent_id, checkpoint_id)
            .await
    }
...
mod tests {
    ...
    async fn test_filesystem_persistence_save_load() {
        ...
        let checkpoint_id = persistence
            .save_state(&state, CheckpointReason::PeriodicSave)
            .await
            .unwrap();
    ...
    async fn test_checkpoint_creation_and_restoration() {
        ...
        // Create checkpoint
        let checkpoint_id = persistence
            .create_checkpoint(&state, CheckpointReason::BeforeRecovery)
            .await
            .unwrap();
        assert!(!checkpoint_id.is_empty());
        ...
        let restored_state = persistence
            .restore_from_checkpoint(&state.agent_id, &checkpoint_id)
            .await
            .unwrap();
    ...
    async fn test_checkpoint_listing() {
        ...
        let _checkpoint1 = persistence
            .create_checkpoint(&state, CheckpointReason::PeriodicSave)
            .await
            .unwrap();
        let _checkpoint2 = persistence
            .create_checkpoint(&state, CheckpointReason::BeforeRecovery)
            .await
            .unwrap();
    ...
    async fn test_state_persistence_manager() {
        ...
        let checkpoint_id = manager
            .save_state(&state, CheckpointReason::UserRequested)
            .await
            .unwrap();
        assert!(!checkpoint_id.is_empty());
