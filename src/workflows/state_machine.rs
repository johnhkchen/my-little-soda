...
impl StateMachine {
    ...
    pub async fn execute_atomic_transition(
        &self,
        ...
        match transition {
            StateTransition::AssignToAgent {
                agent_id,
                issue_url,
            } => self.atomic_assign_to_agent(&agent_id, &issue_url).await,
            StateTransition::StartWork {
                agent_id,
                issue_url,
            } => self.atomic_start_work(&agent_id, &issue_url).await,
            StateTransition::CompleteWork {
                agent_id,
                issue_url,
            } => self.atomic_complete_work(&agent_id, &issue_url).await,
            StateTransition::StartReview {
                agent_id,
                issue_url,
                pr_number,
            } => {
                self.atomic_start_review(&agent_id, &issue_url, pr_number)
                    .await
            }
            StateTransition::StartLanding {
                agent_id,
                issue_url,
            } => self.atomic_start_landing(&agent_id, &issue_url).await,
            StateTransition::IntegrateWork {
                agent_id,
                issue_url,
    ...
    fn parse_issue_number_from_url_static(issue_url: &str) -> Result<u64, GitHubError> {
        ...
        if let Some(issue_number_str) = issue_url.split('/').next_back() {
            issue_number_str
                .parse::<u64>()
                .map_err(|_| GitHubError::TokenNotFound("Invalid issue URL format".to_string()))
        } else {
            Err(GitHubError::TokenNotFound(
                "Could not extract issue number from URL".to_string(),
            ))
        }
    }

    async fn atomic_assign_to_agent(
        &self,
        agent_id: &str,
        issue_url: &str,
    ) -> Result<TransitionResult, GitHubError> {
    ...
    async fn atomic_start_work(
        ...
        println!("🚀 Atomically starting work: agent {agent_id} on issue {issue_url}");
    ...
    async fn atomic_start_review(
        ...
        println!(
            "🔄 Phase 1: Starting review for agent {agent_id} issue {issue_url} (PR #{pr_number})"
        );
    ...
    async fn atomic_integrate_work(
        &self,
        agent_id: &str,
        issue_url: &str,
    ) -> Result<TransitionResult, GitHubError> {
        ...
        println!("🔄 Atomically integrating work: agent {agent_id} issue {issue_url}");
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    Err(GitHubError::IoError(std::io::Error::other(format!(
                        "Failed to remove route:ready label: {error_msg}"
                    ))))
                }
            }
    ...
    async fn complete_final_integration(&self, issue_number: u64) -> Result<(), GitHubError> {
        ...
        match output {
            Ok(result) => {
                if !result.status.success() {
                    let error_msg = String::from_utf8_lossy(&result.stderr);
                    return Err(GitHubError::IoError(std::io::Error::other(format!(
                        "Failed to remove route:ready_to_merge label: {error_msg}"
                    ))));
                }
            }
    ...
    pub async fn validate_state_consistency(&self) -> Result<bool, GitHubError> {
        ...
        println!(
            "🔍 Validating state consistency for {} agents and {} issues",
            agents.len(),
            issues.len()
        );
...
mod tests_parse_issue_number {
    ...
    fn empty_url_is_error() {
        ...
        assert!(
            msg.contains("Invalid issue URL format")
                || msg.contains("Could not extract issue number")
        );
    }
