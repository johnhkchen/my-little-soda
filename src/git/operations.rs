...
use git2::{
    BranchType, Oid, Repository, Signature,
    PushOptions, RemoteCallbacks, Cred, Direction
};
use std::path::Path;
...
impl Git2Operations {
    ...
    fn get_signature(&self) -> Result<Signature> {
        ...
        match self.repo.signature() {
            Ok(sig) => Ok(sig),
            Err(_) => {
                Signature::now("Clambake Agent", "noreply@clambake.dev")
...
impl GitOperations for Git2Operations {
    ...
    fn create_branch(&self, name: &str, from: &str) -> Result<()> {
        ...
        let from_commit = if from == "HEAD" {
            ...
            if let Ok(branch) = self.repo.find_branch(from, BranchType::Local)
        ...
        // Create the branch
        self.repo.branch(name, &from_commit, false)
