                ...
                match client.fetch_issues().await {
                    Ok(issues) => {
                        ...
                        for issue in agent_labeled_issues {
                            let agent_labels: Vec<_> = issue.labels.iter()
                                .filter(|label| label.name.starts_with("agent"))
                                .collect();
                            
                            for agent_label in agent_labels {
                                print!("🧹 Removing {} from Issue #{}... ", agent_label.name, issue.number);
    ...
    match output {
        Ok(result) => {
            if result.status.success() {
                ...
                Err(crate::github::GitHubError::IoError(std::io::Error::new(
        ...
        Err(e) => {
            Err(crate::github::GitHubError::IoError(e))
        }
    }
}