...
impl DoctorCommand {
    ...
    fn check_my_little_soda_config(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        ...
        if std::path::Path::new(".my-little-soda").exists() {
            ...
            checks.insert(
                ...
                DiagnosticResult {
                    ...
                    details: None,
                    suggestion: Some(
                        "Run 'my-little-soda init' to initialize the project".to_string(),
                    ),
                },
            );
        }
    ...
    fn check_environment_variables(
        ...
        match env::var("MY_LITTLE_SODA_GITHUB_TOKEN") {
            ...
            Ok(_) => {
                issues.push(
                    "MY_LITTLE_SODA_GITHUB_TOKEN is empty or has placeholder value".to_string(),
                );
            }
            Err(_) => {
                ...
                {
                    warnings.push(
                        "MY_LITTLE_SODA_GITHUB_TOKEN not set but GitHub CLI is authenticated"
                            .to_string(),
                    );
                } else {
        ...
        if let Ok(path) = env::var("PATH") {
            let has_cargo_bin = path
                .split(':')
                .any(|p| p.contains("cargo/bin") || p.contains(".cargo/bin"));
            if has_cargo_bin {
                good.push("PATH includes Cargo bin directory ✅".to_string());
            } else {
                warnings.push(
                    "PATH may not include Cargo bin directory for global installations".to_string(),
                );
            }
        }
        ...
        for (conflict_var, preferred_var) in conflicting_vars {
            if env::var(conflict_var).is_ok() && env::var(preferred_var).is_err() {
                warnings.push(format!(
                    "{} is set but {} is not - consider using {}",
        ...
        let suggestion = if !issues.is_empty() {
            Some(
                "Set MY_LITTLE_SODA_GITHUB_TOKEN environment variable or run 'gh auth login'"
                    .to_string(),
            )
        } else if !warnings.is_empty() {
    ...
    fn check_file_system_permissions(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        ...
        if soda_dir.exists() {
            ...
            if soda_dir.is_dir() {
                ...
                match fs::write(&test_file, "test") {
                    ...
                    Err(e) => {
                        issues.push(format!("Cannot write to .my-little-soda directory: {}", e))
                    }
                }
            } else {
        ...
        let message = if issues.is_empty() {
            ...
            format!(
                "File system permission issues detected ({} issues)",
                issues.len()
            )
        };
        ...
        let suggestion = if !issues.is_empty() {
            Some(
                "Ensure current user has read/write permissions for the current directory"
                    .to_string(),
            )
        } else {
            None
            ...
            Ok(space_mb) => {
                if space_mb > 1000.0 {
                    ...
                    (
                        ...
                        if self.is_verbose() {
                            Some("Disk space check passed".to_string())
                        } else {
                            None
                        },
                        None,
                    )
                } else if space_mb > 100.0 {
                    // 100MB - 1GB
                    (
                        DiagnosticStatus::Warning,
                    ...
                    (
                        ...
                        Some(
                            "Free up disk space before running My Little Soda operations"
                                .to_string(),
                        ),
                    )
                }
    ...
    fn get_available_space(&self, path: &Path) -> Result<f64> {
        ...
        if max_writable > 0.0 {
            ...
            Err(anyhow::anyhow!(
                "Cannot write even small test files - disk may be full"
            ))
        }
    }

    /// Check PATH configuration if globally installed
    fn check_path_configuration(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        ...
        match Command::new("my-little-soda").arg("--version").output() {
            ...
            _ => {
                ...
                if Path::new(&format!("{}/my-little-soda", cargo_bin)).exists() {
                    warnings.push(format!(
                        "my-little-soda installed in {} but not in PATH",
                        cargo_bin
                    ));
                } else {
                    path_status.push(
                        "my-little-soda not found in PATH (using local build) ℹ️".to_string(),
                    );
                }
            }
        }
        ...
        if let Ok(path_env) = env::var("PATH") {
            ...
            let cargo_paths: Vec<&str> = paths
                .iter()
                .filter(|p| p.contains("cargo"))
                .copied()
                .collect();
            ...
            if !cargo_paths.is_empty() {
                ...
                warnings.push(
                    "No Cargo paths detected in PATH - global installations may not work"
                        .to_string(),
                );
            }
        }
    ...
    fn check_current_directory_access(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        let current_dir = env::current_dir().unwrap_or_else(|_| Path::new(".").to_path_buf());
        ...
        let message = if issues.is_empty() {
            ...
            format!(
                "Current directory access issues detected ({} issues)",
                issues.len()
            )
        };
    ...
    fn check_temporary_directory_access(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        let mut issues = Vec::new();
        ...
        if temp_dir.exists() && temp_dir.is_dir() {
            good.push(format!(
                "System temp directory accessible: {} ✅",
                temp_dir.display()
            ));
        ...
        let message = if issues.is_empty() {
            ...
            format!(
                "Temporary directory access issues detected ({} issues)",
                issues.len()
            )
        };
    ...
    fn check_conflicting_configurations(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) -> Result<()> {
        ...
        let github_tokens = vec![
            (
                ...
                env::var("MY_LITTLE_SODA_GITHUB_TOKEN").ok(),
            ),
            ("GITHUB_TOKEN", env::var("GITHUB_TOKEN").ok()),
            ("GH_TOKEN", env::var("GH_TOKEN").ok()),
        ];

        let set_tokens: Vec<&str> = github_tokens
            .iter()
        ...
        if set_tokens.len() > 1 {
            conflicts.push(format!(
                "Multiple GitHub tokens set: {} - may cause confusion",
                set_tokens.join(", ")
            ));
        ...
        if let Ok(config_owner) = config().map(|c| c.github.owner.clone()) {
            if let Ok(env_owner) = env::var("GITHUB_OWNER") {
                if config_owner != env_owner {
                    conflicts.push(format!(
                        "GitHub owner mismatch: config='{}' vs env='{}'",
                        config_owner, env_owner
                    ));
                }
            }
        }
        ...
        if let Ok(config_repo) = config().map(|c| c.github.repo.clone()) {
            if let Ok(env_repo) = env::var("GITHUB_REPO") {
                if config_repo != env_repo {
                    conflicts.push(format!(
                        "GitHub repo mismatch: config='{}' vs env='{}'",
                        config_repo, env_repo
                    ));
                }
            }
        }
        ...
        if let Ok(config_log) = config().map(|c| c.observability.log_level.clone()) {
            if let Ok(env_log) = env::var("MY_LITTLE_SODA_OBSERVABILITY_LOG_LEVEL") {
                if config_log != env_log {
                    warnings.push(format!(
                        "Log level mismatch: config='{}' vs env='{}'",
                        config_log, env_log
                    ));
                }
            }
        ...
        let message = if !conflicts.is_empty() {
            format!(
                "Configuration conflicts detected ({} conflicts)",
                conflicts.len()
            )
        } else if !warnings.is_empty() {
            format!(
                "Configuration warnings detected ({} warnings)",
                warnings.len()
            )
        } else {
    ...
    fn check_file_operations(&self, checks: &mut HashMap<String, DiagnosticResult>) -> Result<()> {
        ...
        let message = if operations_failed.is_empty() {
            ...
            format!(
                "File operation issues detected ({} failed, {} passed)",
                operations_failed.len(),
                operations_tested.len()
            )
        };
        ...
        match GitHubClient::with_verbose(self.is_verbose()) {
            Ok(github_client) => {
                ...
                let agent_diagnostic = match AgentStateDiagnostic::new(github_client)
                    .with_coordinator_verbose(self.is_verbose())
                    .await
                {
                    ...
                    Err(e) => {
                        checks.insert(
                            ...
                            DiagnosticResult {
                                ...
                                details: Some(format!("Error: {:?}", e)),
                                suggestion: Some(
                                    "Ensure GitHub authentication and configuration are correct"
                                        .to_string(),
                                ),
                            },
                        );
                ...
                let has_failures = checks.values().any(|result| {
                    matches!(result.status, DiagnosticStatus::Fail)
                        ...
                        .iter()
                        .any(|key| checks.contains_key(*key))
                });
    ...
    async fn check_end_to_end_workflow_validation(
        &self,
        checks: &mut HashMap<String, DiagnosticResult>,
    ) {
    ...
    async fn check_agent_lifecycle_readiness(&self) -> DiagnosticResult {
        ...
        if readiness_issues.is_empty() {
            ...
            DiagnosticResult {
                status: DiagnosticStatus::Fail,
                message: format!(
                    "Agent lifecycle not ready ({} issues)",
                    readiness_issues.len()
                ),
                details: Some(format!("Issues: {}", readiness_issues.join("; "))),
                suggestion: Some(
                    "Complete system initialization before starting agent operations".to_string(),
                ),
            }
        }
    }
    ...
    async fn check_issue_workflow_integration(&self) -> DiagnosticResult {
        match crate::github::client::GitHubClient::with_verbose(self.is_verbose()) {
            Ok(client) => {
                    ...
                    .await
                {
                    Ok(issues_page) => {
                        ...
                        let has_ready_issues = issues_page.items.iter().any(|issue| {
                            issue
                                .labels
                                .iter()
                                .any(|label| label.name.starts_with("route:"))
                        });
                        ...
                        let has_agent_assignment = issues_page
                            .items
                            .iter()
                        ...
                        if missing_features.is_empty() || workflow_features.len() > 0 {
                            DiagnosticResult {
                                status: if missing_features.is_empty() {
                                    DiagnosticStatus::Pass
                                } else {
                                    DiagnosticStatus::Warning
                                },
                            ...
                            DiagnosticResult {
                                ...
                                suggestion: Some(
                                    "Add issues with routing labels to test workflow integration"
                                        .to_string(),
                                ),
                            }
                        }
                    ...
                    Err(e) => DiagnosticResult {
                        ...
                        suggestion: Some(
                            "Verify GitHub token has issues access permissions".to_string(),
                        ),
                    },
                }
            }
            Err(e) => DiagnosticResult {
                ...
                suggestion: Some(
                    "Configure GitHub authentication to test issue workflow".to_string(),
                ),
            },
        }
    }
    ...
    async fn check_branch_pr_workflow(&self) -> DiagnosticResult {
        match crate::github::client::GitHubClient::with_verbose(self.is_verbose()) {
            Ok(client) => {
                ...
                {
                    Ok(branches_page) => {
                        let agent_branches: Vec<_> = branches_page
                            .items
                            .iter()
                        ...
                        if !agent_branches.is_empty() {
                            workflow_status
                                .push(format!("{} agent branches found", agent_branches.len()));
                        }
                            ...
                            .await
                        {
                            ...
                            Err(_) => DiagnosticResult {
                                ...
                                suggestion: Some(
                                    "Verify GitHub token has PR access permissions".to_string(),
                                ),
                            },
                        }
                    }
    ...
    async fn check_agent_coordination_readiness(&self) -> DiagnosticResult {
        ...
        if missing_features.is_empty() {
            ...
            DiagnosticResult {
                status: DiagnosticStatus::Fail,
                message: format!(
                    "Agent coordination not ready ({} missing features)",
    ...
    async fn check_complete_workflow_simulation(&self) -> DiagnosticResult {
        ...
        let success_rate =
            simulation_steps.len() as f64 / (simulation_steps.len() + failed_steps.len()) as f64;

        // Create detailed status message
        let message = if failed_steps.is_empty() && warnings.is_empty() {
            format!(
                "Complete workflow simulation successful ({}ms total)",
                total_time.as_millis()
            )
        } else if failed_steps.is_empty() {
            format!(
                "Workflow simulation successful with {} warnings ({}ms total)",
                warnings.len(),
                total_time.as_millis()
            )
        } else if success_rate >= 0.75 {
            format!(
                "Workflow simulation mostly successful ({:.0}%, {}ms total)",
                success_rate * 100.0,
                total_time.as_millis()
            )
        } else {
            format!(
                "Workflow simulation failed ({:.0}% success, {}ms total)",
                success_rate * 100.0,
                total_time.as_millis()
            )
        };
        ...
        let (status, suggestion) = if failed_steps.is_empty() {
            if warnings.is_empty() {
                (
                    DiagnosticStatus::Pass,
                    Some("System ready for full autonomous agent operation".to_string()),
                )
            } else {
                (
                    DiagnosticStatus::Warning,
                    Some("System ready but review warnings for optimal performance".to_string()),
                )
            }
        } else if success_rate >= 0.75 {
            (
                DiagnosticStatus::Warning,
                Some("Address failed workflow steps for reliable agent operation".to_string()),
            )
        } else {
            (
                DiagnosticStatus::Fail,
                Some(
                    "Critical workflow issues must be resolved before agent operations".to_string(),
                ),
            )
        };
    ...
    async fn simulate_pop_operation(&self) -> anyhow::Result<Vec<String>> {
        ...
        if issues.items.is_empty() {
            ...
            let agent_assigned_issues: Vec<_> = issues
                .items
                .iter()
                .filter(|issue| {
        ...
        if let Err(_) =
            crate::agents::coordinator::AgentCoordinator::with_verbose(self.is_verbose()).await
        {
    ...
    async fn simulate_bottle_operation(&self) -> anyhow::Result<Vec<String>> {
        ...
        if let Some(test_issue) = issues.items.first() {
            ...
            let has_transitional_labels = current_labels
                .iter()
                .any(|label| label.starts_with("route:"));

            if !has_transitional_labels {
                warnings
                    .push("No issues with routing labels found for transition testing".to_string());
            }

            // Test agent label removal simulation
            let has_agent_labels = current_labels
                .iter()
        ...
        match repo.head() {
            Ok(head) => {
                if let Some(branch) = head.shorthand() {
                    if branch == "main" {
                        warnings
                            .push("Currently on main branch - no work branch active".to_string());
                    }
                } else {
                    warnings
                        .push("Cannot determine current branch for bottle operation".to_string());
                }
            }
    ...
    async fn simulate_error_recovery(&self) -> anyhow::Result<Vec<String>> {
        ...
        if index_lock.exists() {
            warnings
                .push("Git index lock file exists - potential concurrent operations".to_string());
        }
            ...
            match octocrab.ratelimit().get().await {
                Ok(rate_limits) => {
                    ...
                    if remaining_pct < 20.0 {
                        warnings.push(format!(
                            "Low GitHub API rate limit remaining ({:.1}%)",
                            remaining_pct
                        ));
                    }
                }
    ...
    fn calculate_readiness(
        ...
        checks: &HashMap<String, DiagnosticResult>,
    ) -> SystemReadiness {
        ...
        let critical_failures = critical_checks
            .iter()
            .filter(|&check| {
                checks
                    .get(*check)
        ...
        let (status, description) = if critical_failures > 0 {
            ...
            (
                ReadinessStatus::PartiallyReady,
                format!(
                    "System partially ready: {} issues need attention",
                    summary.failed
                ),
            )
    ...
    fn generate_recommendations(
        ...
        if recommendations.len() < 3 {
            recommendations.push(ActionableRecommendation {
                ...
                description: "Enable advanced autonomous agent features for better productivity"
                    .to_string(),
                steps: vec![
