        ...
        Ok(client) => {
            ...
            .await
            {
                Ok(Ok(labels_page)) => {
                    ...
                    for required_label in &required_labels {
                        if labels_page
                            .items
                            .iter()
                            .any(|label| label.name == required_label.name)
                        {
                    ...
                    if missing_labels.is_empty() {
                        ...
                        DiagnosticResult {
                            ...
                            suggestion: Some(
                                "Run 'my-little-soda init' to create missing labels automatically"
                                    .to_string(),
                            ),
                        }
                    }
                }
                Ok(Err(_)) => DiagnosticResult {
                    status: DiagnosticStatus::Fail,
                    message: "Cannot access repository labels".to_string(),
                    details: Some("GitHub API error when listing labels".to_string()),
                    suggestion: Some(
                        "Check GitHub authentication and repository access".to_string(),
                    ),
                },
                Err(_) => DiagnosticResult {
                    ...
                    suggestion: Some(
                        "Check network connectivity and GitHub authentication".to_string(),
                    ),
                },
            }
        }
        ...
        Ok(client) => {
            ...
            .await
            {
                Ok(Ok(labels_page)) => {
                    ...
                    for required_label in &required_labels {
                        if let Some(existing_label) = labels_page
                            .items
                            .iter()
                            .find(|label| label.name == required_label.name)
                        {
                            let mut issues = Vec::new();

                            if existing_label.color != required_label.color {
                                issues.push(format!(
                                    "color mismatch (expected: #{}, actual: #{})",
                                    required_label.color, existing_label.color
                                ));
                            }

                            if existing_label.description.as_deref()
                                != Some(&required_label.description)
                            {
                            ...
                            if issues.is_empty() {
                                ...
                                configuration_issues.push(format!(
                                    "{}: {}",
                                    required_label.name,
                                    issues.join(", ")
                                ));
...
pub async fn check_label_management_capabilities(verbose: bool) -> DiagnosticResult {
    match crate::github::client::GitHubClient::with_verbose(verbose) {
        Ok(client) => {
            ...
            let create_result = octocrab
                ...
                .create_label(
                    &test_label_name,
                    "ffffff",
                    "Test label for management capability validation",
                )
                .await;
                    ...
                    let update_result = async {
                        ...
                        octocrab
                            ...
                            .create_label(
                                &test_label_name,
                                "000000",
                                "Updated test label description",
                            )
                            .await
                    }
                    .await;
            ...
            if failed_operations.is_empty() {
                ...
                DiagnosticResult {
                    status: DiagnosticStatus::Warning,
                    message: format!(
                        "Some label management operations failed ({}/3 successful)",
                    ...
                    suggestion: Some(
                        "Check repository write permissions and GitHub token scopes".to_string(),
                    ),
                }
            } else {
                DiagnosticResult {
                    ...
                    suggestion: Some(
                        "Ensure GitHub token has repository write permissions".to_string(),
                    ),
                }
            }
        }
        Err(e) => DiagnosticResult {
            ...
            details: Some(format!("GitHub client error: {:?}", e)),
            suggestion: Some(
                "Configure GitHub authentication to test label management".to_string(),
            ),
        },
    }
}
                ...
                Ok(Ok(repo)) => {
                    ...
                    if has_write_access {
                        ...
                        DiagnosticResult {
                            ...
                            details: Some(
                                "Label management operations may fail without write permissions"
                                    .to_string(),
                            ),
                            suggestion: Some(
                                "Request repository write access to use label management features"
                                    .to_string(),
                            ),
                        }
                    }
                }
                Ok(Err(e)) => DiagnosticResult {
                    status: DiagnosticStatus::Fail,
                ...
                .await
            {
                Ok(issues_page) => {
                    ...
                    for issue in &issues_page.items {
                        ...
                        let routing_label_count = issue_labels
                            .iter()
                            .filter(|label| routing_labels.contains(&label.as_str()))
                            .count();

                        if routing_label_count > 1 {
                            let conflicting_labels: Vec<String> = issue_labels
                                .iter()
                                .filter(|label| routing_labels.contains(&label.as_str()))
                                .cloned()
                                .collect();
                            problems.push(format!(
                                "Issue #{}: has {} conflicting routing labels: {}",
                                issue.number,
                                routing_label_count,
                        ...
                        if routing_label_count == 0
                            && !issue_labels.contains(&"route:human-only".to_string())
                        {
                            // Skip issues assigned to agents (they're in progress)
                            let has_agent_label =
                                issue_labels.iter().any(|label| label.starts_with("agent"));
                            if !has_agent_label {
                        ...
                        if has_agent_label && has_ready_label {
                            problems.push(format!(
                                "Issue #{}: assigned to agent but still has route:ready label",
                                issue.number
                            ));
                        }
                    }
                    ...
                    if problems.is_empty() {
                        DiagnosticResult {
                            status: DiagnosticStatus::Pass,
                            message: format!(
                                "All {} open issues have proper label states",
                                total_issues
                            ),
                            details: if verbose {
                                Some(format!(
                                    "Checked {} issues for routing label compliance",
                                    total_issues
                                ))
                            } else {
                                None
...
pub async fn check_workflow_label_compliance(verbose: bool) -> DiagnosticResult {
    match crate::github::client::GitHubClient::with_verbose(verbose) {
                ...
                .await
            {
                Ok(issues_page) => {
                    ...
                    for issue in &issues_page.items {
                        ...
                        let has_priority = issue_labels
                            .iter()
                            .any(|label| label.starts_with("route:priority-"));
                        let has_routing =
                            issue_labels.iter().any(|label| label.starts_with("route:"));
                            ...
                            && !issue_labels.contains(&"route:human-only".to_string())
                        {
                            compliance_issues.push(format!(
                                "Issue #{}: has routing label but missing priority",
                                issue.number
                            ));
                        }
                    }
                    ...
                    if compliance_issues.is_empty() {
                        DiagnosticResult {
                            ...
                            details: if verbose {
                                Some(format!(
                                    "Workflow state: {} assigned to agents, {} ready, {} in review",
                        ...
                        DiagnosticResult {
                            status: DiagnosticStatus::Warning,
                            message: format!(
                                "{} workflow compliance issues found",
                                compliance_issues.len()
                            ),
                            details: Some(compliance_issues.join("; ")),
                            suggestion: Some(
                                "Add missing priority labels to routing-enabled issues".to_string(),
                            ),
                        }
                    }
                }
        ...
        Err(e) => DiagnosticResult {
            ...
            details: Some(format!("GitHub client error: {:?}", e)),
            suggestion: Some(
                "Configure GitHub authentication to check workflow compliance".to_string(),
            ),
        },
    }
}
