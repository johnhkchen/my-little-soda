...
pub trait StateValidation {
    ...
    async fn check_specific_pattern(&self, pattern_type: &str) -> Result<Vec<StuckAgentPattern>, StateError>;
}
...
impl StateValidator {
    ...
    async fn check_agent_branch_exists(&self, agent_id: &str, issue_number: u64) -> Result<bool, StateError> {
        ...
        match std::process::Command::new("git")
            .args(&["branch", "-r", "--list", &format!("origin/{}*", expected_branch_prefix)])
            .output()
        {
            ...
            Ok(_) => Ok(false),
            Err(e) => Err(StateError::GitError(format!("Failed to check git branches: {}", e))),
        }
    }

    /// Check if there are any commits ahead on the agent's branch
    async fn check_commits_ahead(&self, agent_id: &str, issue_number: u64) -> Result<Option<u32>, StateError> {
        let expected_branch_prefix = format!("{}/{}", agent_id, issue_number);
        ...
        if let Some(branch_line) = branch_line {
            let branch_name = branch_line.trim().strip_prefix("origin/").unwrap_or(branch_line.trim());
            ...
            if commits_output.status.success() {
                let count_str = String::from_utf8_lossy(&commits_output.stdout).trim().to_string();
    ...
    async fn detect_labeled_but_no_branch(&self) -> Result<Vec<StuckAgentPattern>, StateError> {
        let issues = self.github_client.fetch_issues_with_state(Some(octocrab::params::State::Open))
            .await
        ...
        for issue in issues {
            if let Some(agent_id) = Self::extract_agent_id_from_labels(&issue.labels) {
                let has_branch = self.check_agent_branch_exists(&agent_id, issue.number).await?;
...
impl StateValidation for StateValidator {
    async fn validate_agent_state(&self, agent_id: &str) -> Result<ValidationReport, StateError> {
        ...
        let agent_issues: Vec<_> = issues.into_iter()
            .filter(|issue| {
    ...
    async fn detect_all_inconsistencies(&self) -> Result<Vec<Inconsistency>, StateError> {
        ...
        let inconsistencies = all_patterns.into_iter().map(|pattern| {
    ...
    async fn check_specific_pattern(&self, pattern_type: &str) -> Result<Vec<StuckAgentPattern>, StateError> {
        match pattern_type {
            ...
            _ => Err(StateError::ValidationError(format!("Unknown pattern type: {}", pattern_type))),
        }
    }
}
...
mod tests {
    ...
    fn test_stuck_agent_patterns_serialization() {
        ...
        for pattern in patterns {
            ...
            let deserialized: StuckAgentPattern = serde_json::from_str(&serialized).expect("Deserialization should work");
            
            match (&pattern, &deserialized) {
                (
                    StuckAgentPattern::LabeledButNoBranch { agent_id: a1, issue: i1 },
                    StuckAgentPattern::LabeledButNoBranch { agent_id: a2, issue: i2 }
                ) => {
                    assert_eq!(a1, a2);
                    assert_eq!(i1, i2);
                },
                (
                    StuckAgentPattern::BranchButNoLabel { agent_id: a1, branch: b1 },
                    StuckAgentPattern::BranchButNoLabel { agent_id: a2, branch: b2 }
                ) => {
                    assert_eq!(a1, a2);
                    assert_eq!(b1, b2);
                },
                (
                    StuckAgentPattern::WorkingButNoCommits { agent_id: a1, issue: i1 },
                    StuckAgentPattern::WorkingButNoCommits { agent_id: a2, issue: i2 }
                ) => {
                    assert_eq!(a1, a2);
                    assert_eq!(i1, i2);
                },
    ...
    fn test_inconsistency_serialization() {
        ...
        let deserialized: Inconsistency = serde_json::from_str(&serialized).expect("Deserialization should work");
