...
use super::validation::{Inconsistency, StuckAgentPattern, StateValidator, StateValidation};

/// Recovery action outcomes
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RecoveryAction {
    RemoveLabel { agent_id: String, issue: u64 },
    CreateBranch { agent_id: String, issue: u64, branch_name: String },
    AddLabel { agent_id: String, issue: u64 },
    CleanBranch { agent_id: String, branch_name: String },
    ResetToAssigned { agent_id: String, issue: u64 },
    ForceReset { agent_id: String },
}
...
pub trait AutomaticRecovery {
    ...
    async fn recover_all_inconsistencies(&self) -> Result<ComprehensiveRecoveryReport, RecoveryError>;
    
    /// Recover from a specific inconsistency pattern
    async fn recover_inconsistency(&self, inconsistency: &Inconsistency) -> Result<RecoveryAttempt, RecoveryError>;
    ...
    async fn can_safely_recover(&self, inconsistency: &Inconsistency) -> Result<bool, RecoveryError>;
}
...
impl AutoRecovery {
    ...
    async fn recover_labeled_but_no_branch(&self, agent_id: &str, issue: u64) -> Result<RecoveryAttempt, RecoveryError> {
        let now = Utc::now();
        
        // First check if we can safely recover
        if !self.can_safely_recover(&Inconsistency {
        ...
        match self.github_client.create_branch(&branch_name, "main").await {
            ...
            Err(e) => {
                ...
                match self.github_client.issues.remove_label(issue, agent_id).await {
                    Ok(_) => {
                        Ok(RecoveryAttempt {
                            action: RecoveryAction::RemoveLabel { 
                                agent_id: agent_id.to_string(), 
                    ...
                    Err(remove_err) => {
                        Ok(RecoveryAttempt {
                            ...
                            success: false,
                            error: Some(format!("Branch creation failed: {}, Label removal failed: {}", e, remove_err)),
                            attempted_at: now,
                            rollback_info: None,
                        })
                    }
    ...
    async fn recover_branch_but_no_label(&self, agent_id: &str, branch_name: &str) -> Result<RecoveryAttempt, RecoveryError> {
        let now = Utc::now();
                ...
                match self.github_client.issues.add_label(issue, agent_id).await {
                    ...
                    Err(e) => {
                        ...
                        return Ok(RecoveryAttempt {
                            ...
                            success: false,
                            error: Some(format!("Cannot add label and will not clean branch with work: {}", e)),
                            attempted_at: now,
                            rollback_info: None,
                        });
                    }
    ...
    async fn recover_working_but_no_commits(&self, agent_id: &str, issue: u64) -> Result<RecoveryAttempt, RecoveryError> {
        let now = Utc::now();
...
impl AutomaticRecovery for AutoRecovery {
    async fn recover_all_inconsistencies(&self) -> Result<ComprehensiveRecoveryReport, RecoveryError> {
        ...
        let inconsistencies = self.validator.detect_all_inconsistencies().await
            .map_err(|e| RecoveryError::ValidationError(format!("Failed to detect inconsistencies: {:?}", e)))?;
        ...
        for inconsistency in &inconsistencies {
            match self.recover_inconsistency(inconsistency).await {
                Ok(attempt) => {
                    if attempt.success {
                        recovered.push(format!("{}: {:?}", inconsistency.agent_id, inconsistency.pattern));
    ...
    async fn rollback_recovery(&self, rollback_info: &RollbackInfo) -> Result<(), RecoveryError> {
        ...
        let rollback_inconsistency = match &rollback_info.rollback_action {
            RecoveryAction::RemoveLabel { agent_id, issue } => {
                Inconsistency {
                    agent_id: agent_id.clone(),
                    pattern: StuckAgentPattern::LabeledButNoBranch { 
                        agent_id: agent_id.clone(), 
            ...
            RecoveryAction::CreateBranch { agent_id, issue, .. } => {
                Inconsistency {
                    agent_id: agent_id.clone(),
                    pattern: StuckAgentPattern::LabeledButNoBranch { 
                        agent_id: agent_id.clone(), 
            ...
            RecoveryAction::AddLabel { agent_id, issue } => {
                Inconsistency {
                    agent_id: agent_id.clone(),
            ...
            RecoveryAction::CleanBranch { agent_id, branch_name } => {
                Inconsistency {
                    agent_id: agent_id.clone(),
                    pattern: StuckAgentPattern::BranchButNoLabel { 
                        agent_id: agent_id.clone(), 
            ...
            _ => return Err(RecoveryError::RollbackFailed("Unsupported rollback action".to_string())),
        };
        ...
        if !rollback_attempt.success {
            return Err(RecoveryError::RollbackFailed(
                rollback_attempt.error.unwrap_or_else(|| "Unknown rollback failure".to_string())
            ));
        }
...
mod tests {
    ...
    fn test_recovery_action_serialization() {
        ...
        for action in actions {
            ...
            let deserialized: RecoveryAction = serde_json::from_str(&serialized).expect("Deserialization should work");
    ...
    fn test_recovery_attempt_structure() {
        ...
        let deserialized: RecoveryAttempt = serde_json::from_str(&serialized).expect("Deserialization should work");
    ...
    fn test_comprehensive_recovery_report() {
        let report = ComprehensiveRecoveryReport {
            ...
            failed: vec![("agent002: BranchButNoLabel".to_string(), "Permission denied".to_string())],
        ...
        let deserialized: ComprehensiveRecoveryReport = serde_json::from_str(&serialized).expect("Deserialization should work");
        
        assert_eq!(report.recovery_rate, deserialized.recovery_rate);
        assert_eq!(report.total_inconsistencies, deserialized.total_inconsistencies);
    }
