use crate::github::{GitHubError, GitHubClient};
...
impl RoutingCoordinator {
    ...
    pub async fn route_issues_to_agents(&self, coordinator: &AgentCoordinator, github_client: &GitHubClient) -> Result<Vec<RoutingAssignment>, GitHubError> {
        ...
        async move {
            ...
            let mut issues = self.issue_filter.fetch_routable_issues(github_client).await?;
    ...
    pub async fn pop_any_available_task(&self, coordinator: &AgentCoordinator, github_client: &GitHubClient, current_user: &str) -> Result<Option<RoutingAssignment>, GitHubError> {
        let routing_start = Instant::now();
        let correlation_id = generate_correlation_id();
        
        let all_issues = github_client.fetch_issues().await?;
        let available_issues = self.issue_filter.filter_available_issues(&all_issues, current_user);
    ...
    pub async fn pop_task_assigned_to_me(&self, coordinator: &AgentCoordinator, github_client: &GitHubClient, current_user: &str) -> Result<Option<RoutingAssignment>, GitHubError> {
        let correlation_id = generate_correlation_id();
        ...
        async move {
            ...
            let mut my_issues = self.issue_filter.filter_assigned_issues(&all_issues, current_user);
    ...
    pub async fn route_specific_issue(&self, coordinator: &AgentCoordinator, github_client: &GitHubClient, issue_number: u64) -> Result<Option<RoutingAssignment>, GitHubError> {
